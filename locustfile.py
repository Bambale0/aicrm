...
class AICRMUser(HttpUser):
    ...
    def view_customers(self):
        ...
        params = {"skip": random.randint(0, 100), "limit": random.randint(10, 50)}

        with self.client.get(
            "/api/customers", params=params, name="view_customers"
        ) as response:
            if response.status_code == 200:
                data = response.json()
    ...
    def create_customer(self):
        ...
        customer_data = {
            ...
            "company": (
                f"Test Company {random.randint(1, 100)}"
        ...
        with self.client.post(
            "/api/customers", json=customer_data, name="create_customer"
        ) as response:
    ...
    def view_orders(self):
        ...
        with self.client.get(
            "/api/orders", params=params, name="view_orders"
        ) as response:
            if response.status_code == 200:
                data = response.json()
                if isinstance(data, list) and len(data) > 0:
    ...
    def create_order(self):
        ...
        order_data = {
            ...
            "print_type": random.choice(
                ["screen_print", "dtf", "embroidery", "heat_transfer"]
            ),
        ...
        with self.client.post(
            "/api/orders", json=order_data, name="create_order"
        ) as response:
            if response.status_code == 201:
                data = response.json()
                if "id" in data:
    ...
    def ai_completion(self):
        ...
        with self.client.post(
            "/api/ai/completions", json=ai_request, name="ai_completion"
        ) as response:
    ...
    def detailed_health(self):
        ...
        if self.user_role == "admin":
            with self.client.get(
                "/health/detailed", name="detailed_health"
            ) as response:
                if response.status_code != 200:
    ...
    def view_analytics(self):
        ...
        if self.user_role in ["admin", "manager"]:
            with self.client.get(
                "/api/analytics/overview", name="analytics_overview"
            ) as response:
