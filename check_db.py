...
def check_database():
    ...
    try:
        ...
        if cursor.fetchone():
            ...
            cursor.execute(
                "SELECT event_type, COUNT(*) FROM triggers GROUP BY event_type;"
            )
        ...
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='robot_actions_config';"
        )
        if cursor.fetchone():
            print("Таблица robot_actions_config существует")

            cursor.execute(
                "SELECT action_type, COUNT(*) FROM robot_actions_config GROUP BY action_type;"
            )
            results = cursor.fetchall()
