        ...
        try:
            ...
            existing_org = (
                ...
                .filter(
                    ...
                    | (Organization.slug == self._generate_slug(name))
                )
                .first()
            )
            ...
            logger.info(
                f"Organization created: {organization.name} (ID: {organization.id})"
            )
            return organization
    ...
    def _create_organization_database(self, org: Organization, db_password: str):
        ...
        try:
            ...
            with admin_engine.connect() as conn:
                ...
                conn.execute(
                    f"CREATE USER {org.db_username} WITH PASSWORD '{db_password}'"
                )
                ...
                conn.execute(
                    f"GRANT ALL PRIVILEGES ON DATABASE {org.db_name} TO {org.db_username}"
                )
                conn.commit()
        ...
        try:
            ...
            with SessionLocal() as org_db:
                admin_user = UserCreate(
                    ...
                    username=org.slug,
                    full_name=org.name,
                    password=decrypt_data(org.db_password_encrypted)[
                        :12
                    ],  # Пароль организации
                    is_active=True,
                    is_superuser=True,
            ...
            logger.error(
                f"Failed to initialize database for organization {org.name}: {e}"
            )
