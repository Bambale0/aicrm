...
class CachedCustomerService(BaseCustomerService):
    ...
    async def get_customer_cached(db: Session, customer_id: int) -> Optional[dict]:
        ...
        if customer:
            ...
            await cache_service.set(
                cache_key, customer_dict, CachedCustomerService.CACHE_TTL_CUSTOMER
            )
    ...
    async def get_customers_cached(
        ...
        if cached_data:
            logger.debug(
                "Customers list cache hit", skip=skip, limit=limit, search=search
            )
            return cached_data
        ...
        # Кешируем
        await cache_service.set(
            cache_key, customers_list, CachedCustomerService.CACHE_TTL_CUSTOMERS_LIST
        )
        logger.debug(
    ...
    async def search_customers_cached(
        ...
        # Кешируем
        await cache_service.set(
            cache_key, customers_list, CachedCustomerService.CACHE_TTL_SEARCH
        )
    ...
    @staticmethod
    async def get_customer_stats_cached(
        ...
        if stats:
            # Кешируем
            await cache_service.set(
                cache_key, stats, CachedCustomerService.CACHE_TTL_STATS
            )
