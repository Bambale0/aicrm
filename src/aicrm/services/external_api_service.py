...
class ExternalAPIService:
                ...
                try:
                    async with httpx.AsyncClient(timeout=self.timeout) as client:
                        ...
                        if response.is_success:
                            ...
                            logger.warning(
                                f"API call failed: {method} {url} -> {response.status_code}: {response.text[:200]}"
                            )
                    ...
                    if attempt == self.max_retries - 1:
                        raise e
                    logger.warning(
                        f"API call attempt {attempt + 1} failed, retrying: {e}"
                    )
                    await asyncio.sleep(
    ...
    async def call_rest_api(
        self, base_url: str, endpoint: str, method: str = "GET", **kwargs
    ) -> Dict[str, Any]:
        """
    ...
    async def call_graphql(
        ...
        return await self.call_api(
            method="POST", url=url, headers=request_headers, json_data=payload
        )
    ...
    async def call_soap_api(
        ...
        return await self.call_api(
            method="POST", url=url, headers=request_headers, data=soap_body
        )

    async def batch_call(
        self, calls: List[Dict[str, Any]], max_concurrent: int = 5
    ) -> List[Dict[str, Any]]:
        """
