...
class SMSService:
    ...
    def __init__(self):
        ...
        self.providers = {
            ...
            "smsru": {
                ...
                "params": {"api_id": self.api_key, "from": self.sender, "json": 1},
            },
        }
    ...
    async def _send_via_smsc(self, phone: str, message: str) -> Dict[str, Any]:
        ...
        params.update({"phones": phone, "mes": message})
    ...
    async def _send_via_twilio(self, phone: str, message: str) -> Dict[str, Any]:
        ...
        data = {"From": provider_config["from"], "To": phone, "Body": message}

        async with httpx.AsyncClient() as client:
            response = await client.post(url, data=data, auth=provider_config["auth"])
            data = response.json()
    ...
    async def _send_via_smsru(self, phone: str, message: str) -> Dict[str, Any]:
        ...
        params = provider_config["params"].copy()
        params.update({"to": phone, "text": message})
    ...
    async def _get_balance_smsc(self) -> Dict[str, Any]:
        ...
        provider_config = self.providers["smsc"]
        params = {"login": self.login, "psw": self.password, "get": "balance", "fmt": 3}
