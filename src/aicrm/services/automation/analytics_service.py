...
from ...models.automation import (
    AutomationExecution, EntityType, RobotAction, TriggerEvent,
    Robot, Trigger, Stage, Process
)
...
class AutomationAnalyticsService:
    ...
    async def get_execution_stats(
        ...
        try:
            ...
            execution_times = [e.execution_time_seconds for e in executions if e.execution_time_seconds]
    ...
    async def get_robot_performance(
        ...
        try:
            query = self.db.query(
                ...
                func.sum(AutomationExecution.actions_successful).label('successful_actions'),
            ...
            for row in results:
                ...
                success_rate = (row.successful_actions / total_actions * 100) if total_actions > 0 else 0

                robots_stats.append({
                    "robot_id": row.robot_id,
                    "robot_name": row.name,
                    "executions_count": row.executions_count,
                    "avg_execution_time_seconds": float(row.avg_time) if row.avg_time else 0,
            ...
            return {
                ...
                "summary": {
                    ...
                    "total_executions": sum(r["executions_count"] for r in robots_stats)
                }
            }
    ...
    async def get_error_analysis(
        ...
        try:
            ...
            failed_executions = query.order_by(desc(AutomationExecution.started_at)).limit(limit).all()
    ...
    async def get_process_efficiency(
        ...
        try:
            ...
            for stage in stages:
                ...
                if start_date:
                    executions_query = executions_query.filter(AutomationExecution.started_at >= start_date)
                if end_date:
                    executions_query = executions_query.filter(AutomationExecution.started_at <= end_date)

                executions = executions_query.all()

                if executions:
                    process_name = stage.process.name if stage.process else f"Process {stage.process_id}"
                    ...
                    # Статистика стадии
                    total_executions = len(executions)
                    successful = sum(1 for e in executions if e.execution_status == "completed")
                    avg_time = sum(e.execution_time_seconds or 0 for e in executions) / total_executions
            ...
            return {
                ...
                "summary": {
                    ...
                    "total_stages": sum(len(p["stages"]) for p in process_stats.values())
                }
            }
        ...
        try:
            ...
            return {
                ...
                "summary": {
                    ...
                    "peak_hour": max(hourly_stats, key=lambda x: x["executions_count"])["hour"] if hourly_stats else None
                }
            }
    ...
    def _classify_error(self, error_message: str) -> str:
        ...
        if "timeout" in error_lower or "time" in error_lower:
            ...
            return "network"
        elif "permission" in error_lower or "access" in error_lower or "auth" in error_lower:
            return "permission"
