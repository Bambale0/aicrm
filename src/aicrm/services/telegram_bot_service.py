...
class TelegramBotService:
    ...
    async def start_polling(self):
        ...
        try:
            ...
            if (
                not hasattr(self.application, "_initialized")
                or not self.application._initialized
            ):
    ...
    async def _handle_orders(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        orders_summary = await self.order_service.get_customer_orders_summary(
            telegram_chat
        )
    ...
    async def _handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        telegram_chat = await self._get_or_create_chat(
        ...
        try:
            ...
            ai_result = await self._process_with_ai(
                message_text, telegram_chat.customer_id
            )
    ...
    async def _process_with_ai(
        self, message: str, customer_id: int = None
    ) -> Dict[str, Any]:
        ...
        result = await self.ai_service.process_customer_message(
            message, customer_context
        )

        return result

    async def _handle_ai_actions(
        self, ai_result: Dict[str, Any], telegram_chat: TelegramChat, update: Update
    ):
        ...
        if intent == IntentType.ORDER:
            ...
            await self._process_order_creation(
                telegram_chat, update.message.text, update
            )
        elif intent == IntentType.COMPLAINT:
            # Эскалация к поддержке
            await self._escalate_to_support(telegram_chat, ai_result)

    async def _process_order_creation(
        self, telegram_chat: TelegramChat, message: str, update: Update
    ):
    ...
    async def _suggest_order_creation(
        self, telegram_chat: TelegramChat, update: Update
    ):
    ...
    async def _escalate_to_support(
        self, telegram_chat: TelegramChat, ai_result: Dict[str, Any]
    ):
        """Эскалация проблемы к поддержке"""
        # Здесь можно отправить уведомление менеджерам
        logger.info(
            f"Эскалация проблемы от клиента {telegram_chat.customer_id}: {ai_result}"
        )
    ...
    async def _get_or_create_chat(self, chat, user) -> TelegramChat:
        ...
        # Поиск в БД
        telegram_chat = (
            self.db.query(TelegramChat).filter(TelegramChat.chat_id == chat_id).first()
        )
    ...
    async def _find_customer_by_telegram_data(self, user) -> Optional[Customer]:
        ...
        if user.first_name and user.last_name:
            ...
            customer = (
                self.db.query(Customer).filter(Customer.name == full_name).first()
            )
            if customer:
                return customer
    ...
    async def _log_communication(
        ...
        communication = Communication(
            ...
            intent=intent,
            extra_data={"chat_id": chat_id, "ai_generated": ai_response},
        )
    ...
    async def _handle_error(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if isinstance(context.error, NetworkError):
            logger.warning(
                "Сетевая ошибка Telegram бота - возможно проблемы с подключением к API Telegram"
            )
    ...
    def get_bot_stats(self) -> Dict[str, Any]:
        ...
        active_chats = (
            self.db.query(TelegramChat).filter(TelegramChat.is_active == True).count()
        )
        total_messages = (
