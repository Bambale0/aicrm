...
class TestAISettingsService:
    ...
    @pytest.mark.asyncio
    async def test_get_user_ai_settings_success(
        self, ai_settings_service, sample_ai_settings
    ):
        """Тест успешного получения настроек AI пользователя"""
        # Мокаем запрос к БД
        ai_settings_service.db.query.return_value.filter.return_value.first.return_value = (
            sample_ai_settings
        )
    ...
    @pytest.mark.asyncio
    async def test_create_ai_settings_success(
        self, ai_settings_service, sample_ai_settings
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_ai_settings_duplicate_user(
        self, ai_settings_service, sample_ai_settings
    ):
    ...
    @pytest.mark.asyncio
    async def test_update_ai_settings_success(
        self, ai_settings_service, sample_ai_settings
    ):
    ...
    @pytest.mark.asyncio
    async def test_delete_ai_settings_success(
        self, ai_settings_service, sample_ai_settings
    ):
    ...
    @pytest.mark.asyncio
    async def test_toggle_ai_settings_active(
        self, ai_settings_service, sample_ai_settings
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_active_ai_settings(
        self, ai_settings_service, sample_ai_settings
    ):
        """Тест получения активных настроек AI"""
        # Мокаем запрос активных настроек
        ai_settings_service.db.query.return_value.filter.return_value.all.return_value = [
            sample_ai_settings
        ]
    ...
    async def test_validate_ai_settings_invalid_temperature(self, ai_settings_service):
        ...
        assert result["valid"] is False
        assert any(
            "Temperature must be between 0 and 2" in error for error in result["errors"]
        )
    ...
    async def test_validate_ai_settings_invalid_max_tokens(self, ai_settings_service):
        ...
        assert result["valid"] is False
        assert any(
            "Max tokens exceeds model limit" in error for error in result["errors"]
        )
    ...
    @pytest.mark.asyncio
    async def test_backup_user_settings_success(
        self, ai_settings_service, sample_ai_settings
    ):
    ...
    @pytest.mark.asyncio
    async def test_restore_user_settings_success(
        self, ai_settings_service, sample_ai_settings
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_user_settings_summary(
        self, ai_settings_service, sample_ai_settings
    ):
    ...
    def test_is_settings_changed(self, ai_settings_service, sample_ai_settings):
        ...
        # Без изменений
        assert not ai_settings_service._is_settings_changed(
            sample_ai_settings,
            {
                "provider": "openai",
                "model": "gpt-4",
        ...
        # С изменениями
        assert ai_settings_service._is_settings_changed(
            sample_ai_settings,
            {
                "provider": "openai",
                "model": "gpt-4-turbo",
    ...
    def test_get_settings_audit_trail(self, ai_settings_service):
        ...
        with patch.object(
            ai_settings_service, "_get_audit_entries", return_value=mock_audit_entries
        ):
