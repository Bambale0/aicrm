...
class TestRobotService:
    ...
    async def test_execute_stage_robots_no_robots(self, robot_service, mock_db):
        ...
        result = await robot_service.execute_stage_robots(EntityType.CUSTOMER, 1, 1)
    ...
    async def test_execute_stage_robots_with_robots(self, robot_service, mock_db):
        ...
        with patch.object(
            robot_service, "_execute_robot_sequence", new_callable=AsyncMock
        ) as mock_execute:
    ...
    async def test_execute_robot_sequence_with_actions(self, robot_service, mock_db):
        ...
        with patch.object(
            robot_service, "_execute_robot_action", new_callable=AsyncMock
    ...
    async def test_execute_robot_action_success(self, robot_service, mock_db):
        ...
        with patch.object(
            robot_service, "_execute_send_email", new_callable=AsyncMock
        ) as mock_executor:
    ...
    async def test_execute_send_email(self, robot_service, mock_db):
        ...
        action_config.config = {"template": "welcome", "email": "test@example.com"}

        with (
            patch.object(
                robot_service, "_get_entity_data", new_callable=AsyncMock
            ) as mock_get_data,
            patch.object(
                robot_service, "_render_email_template", new_callable=AsyncMock
            ) as mock_render,
            patch.object(
                robot_service, "_get_recipient_email", new_callable=AsyncMock
            ) as mock_get_email,
        ):
    ...
    async def test_execute_send_sms(self, robot_service, mock_db):
        """Тест отправки SMS"""
        action_config = MagicMock()
        action_config.config = {"message": "Hello {name}", "phone": "+1234567890"}
        ...
        with (
            ...
            patch.object(
                robot_service, "_get_recipient_phone", new_callable=AsyncMock
            ) as mock_get_phone,
            patch(
                "src.aicrm.services.automation.robot_service.sms_service"
            ) as mock_sms_service,
        ):

            mock_get_data.return_value = {"name": "John"}
            mock_get_phone.return_value = "+1234567890"
            mock_sms_service.send_sms = AsyncMock(
                return_value={
                    "success": True,
                    "provider": "twilio",
                    "message_id": "msg123",
    ...
    async def test_execute_create_task(self, robot_service, mock_db):
        ...
        with patch(
            "src.aicrm.services.automation.robot_service.TaskService"
    ...
    async def test_execute_update_field_customer(self, robot_service, mock_db):
        ...
        action_config = MagicMock()
        action_config.config = {"field": "loyalty_level", "value": "gold"}
    ...
    async def test_execute_update_field_invalid_field(self, robot_service, mock_db):
        ...
        action_config = MagicMock()
        action_config.config = {"field": "", "value": "test"}  # Пустое поле
    ...
    async def test_execute_call_external_api(self, robot_service, mock_db):
        ...
        with (
            ...
            patch(
                "src.aicrm.services.automation.robot_service.external_api_service"
            ) as mock_api_service,
        ):

            mock_get_data.return_value = {"id": 123, "name": "John"}
            mock_api_service.call_api = AsyncMock(
                return_value={
                    "success": True,
                    "status_code": 200,
                    "json": {"result": "ok"},
    ...
    async def test_execute_create_calendar_event(self, robot_service, mock_db):
        ...
        with (
            ...
            patch(
                "src.aicrm.services.automation.robot_service.calendar_service"
    ...
    def test_replace_placeholders_in_dict(self, robot_service):
        ...
        data = {
            "message": "Hello {name}",
            "user_id": "{id}",
            "nested": {"text": "Welcome {name} with ID {id}"},
    ...
    async def test_execute_robot_sequence_error_handling(self, robot_service, mock_db):
        ...
        with (
            ...
            patch.object(
                robot_service.error_handler, "handle_error", new_callable=AsyncMock
