...
class TestAvitoIntegrationService:
    ...
    async def test_handle_message_received_success(
        self, avito_integration_service, mock_db, sample_message_data
    ):
        ...
        result = await avito_integration_service.handle_message_received(
            chat_id="chat_456", message_data=sample_message_data
        )
        ...
        mock_avito_handler.save_message.assert_called_once_with(
            chat_id="chat_456", message=sample_message_data, direction="inbound"
        )
    ...
    async def test_handle_chat_created_success(
        self, avito_integration_service, mock_db, sample_chat_data
    ):
        ...
        result = await avito_integration_service.handle_chat_created(
            chat_id="chat_456", chat_data=sample_chat_data
        )
    ...
    @pytest.mark.asyncio
    async def test_handle_chat_closed_success(self, avito_integration_service, mock_db):
        ...
        result = await avito_integration_service.handle_chat_closed(
            chat_id="chat_456", close_data={"reason": "completed"}
        )
    ...
    async def test_generate_auto_response_success(
        self, avito_integration_service, mock_db
    ):
        ...
        mock_avito_handler.send_message.return_value = {
            "success": True,
            "message_id": "msg_789",
        }
        avito_integration_service.avito_handler = mock_avito_handler
        ...
        result = await avito_integration_service.generate_auto_response(
            chat_id="chat_456", message_text="Сколько стоит товар?"
        )
    ...
    async def test_send_standard_response_success(
        ...
        result = await avito_integration_service.send_standard_response(
            chat_id="chat_456", response_type="greeting"
        )
    ...
    async def test_send_standard_response_unknown_type(
        ...
        # Выполнение теста
        result = await avito_integration_service.send_standard_response(
            chat_id="chat_456", response_type="unknown_type"
        )
...
class TestAutomationServiceAvitoIntegration:
    ...
    @pytest.mark.asyncio
    async def test_on_avito_message_received(self, automation_service, mock_db):
    ...
    @pytest.mark.asyncio
    async def test_on_avito_chat_created(self, automation_service, mock_db):
        ...
        result = await automation_service.on_avito_chat_created(
            chat_id="chat_456", chat_data={"user_id": "user_123"}
        )
    ...
    @pytest.mark.asyncio
    async def test_on_avito_chat_closed(self, automation_service, mock_db):
...
class TestAvitoRobotActions:
    ...
    def sample_action_config(self):
        ...
        from src.aicrm.models.automation import RobotAction, RobotActionConfig

        config = MagicMock(spec=RobotActionConfig)
        config.action_type = RobotAction.GENERATE_AI_RESPONSE
        config.config = {"chat_id": "chat_456", "message_text": "Test message"}
        return config

    @pytest.mark.asyncio
    async def test_execute_generate_ai_response_success(
        self, avito_robot_actions, mock_db, sample_action_config
    ):
        ...
        with patch(
            "src.aicrm.services.automation.avito_robot_actions.AvitolntegrationService",
            return_value=mock_integration_service,
        ):
            # Выполнение теста
            result = await avito_robot_actions._execute_generate_ai_response(
                sample_action_config, EntityType.COMMUNICATION, 1
            )
    ...
    async def test_execute_send_standard_response_success(
        self, avito_robot_actions, mock_db
    ):
        ...
        config.config = {"chat_id": "chat_456", "response_type": "greeting"}
        ...
        with patch(
            ...
            result = await avito_robot_actions._execute_send_standard_response(
                config, EntityType.COMMUNICATION, 1
            )
