...
from ..models.automation import EntityType, RobotAction
...
class TestAutomationErrorHandler:
    ...
    async def test_handle_error_basic(self, error_handler, mock_db):
        ...
        with patch(
            "src.aicrm.services.automation.error_handler.AutomationError",
            return_value=mock_error_record,
        ):
        ...
        assert (
            result["retry_scheduled"] is False
        )  # network ошибки не retry по умолчанию
    ...
    async def test_handle_error_with_retry(self, error_handler, mock_db):
        ...
        with (
            ...
            patch.object(
                error_handler, "_schedule_retry", new_callable=AsyncMock
            ) as mock_schedule,
        ):
    ...
    async def test_handle_error_with_admin_notification(self, error_handler, mock_db):
        ...
        with (
            ...
            patch.object(
                error_handler, "_notify_administrators", new_callable=AsyncMock
            ) as mock_notify,
        ):
    ...
    async def test_process_pending_retries_successful(self, error_handler, mock_db):
        ...
        with patch.object(
            error_handler, "_execute_retry", new_callable=AsyncMock
        ) as mock_execute:
    ...
    @pytest.mark.asyncio
    async def test_process_pending_retries_max_retries_exceeded(
        self, error_handler, mock_db
    ):
    ...
    async def test_execute_retry_robot_action(self, error_handler, mock_db):
        ...
        with patch(
            "src.aicrm.services.automation.robot_service.RobotService"
        ) as mock_robot_service_class:
            mock_robot_service = MagicMock()
            mock_robot_service_class.return_value = mock_robot_service
            mock_robot_service.execute_robot_action = AsyncMock(
    ...
    async def test_execute_retry_trigger_event(self, error_handler, mock_db):
        ...
        with patch(
            "src.aicrm.services.automation.automation_service.AutomationService"
        ) as mock_automation_service_class:
            mock_automation_service = MagicMock()
            mock_automation_service_class.return_value = mock_automation_service
            mock_automation_service.handle_event = AsyncMock(
                return_value={"success": True}
            )
    ...
    def test_calculate_next_retry_time(self, error_handler):
        ...
        with patch(
            "src.aicrm.services.automation.error_handler.datetime"
        ) as mock_datetime:
    ...
    async def test_notify_administrators(self, error_handler, mock_db):
        ...
        with patch.object(
            error_handler.email_service, "send_email", new_callable=AsyncMock
        ) as mock_send:
    ...
    async def test_error_handling_in_handle_error(self, error_handler, mock_db):
        ...
        result = await error_handler.handle_error(
            automation_execution_id=1, error_message="Test error"
        )
    ...
    @pytest.mark.asyncio
    async def test_error_handling_in_process_pending_retries(
        self, error_handler, mock_db
    ):
