...
class TestAvitoTaskIntegration:
    ...
    def mock_task_service(self):
        """Мок для TaskService"""
        with patch(
            "src.aicrm.services.automation.avito_integration.TaskService"
        ) as mock:
    ...
    @pytest.mark.asyncio
    async def test_analyze_message_for_tasks_urgent_help(
        self, task_integration, sample_message_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_analyze_message_for_tasks_complaint(
        self, task_integration, sample_message_data
    ):
        ...
        complaint_message["message"][
            "text"
        ] = "Очень недоволен качеством печати, все размазано!"
    ...
    @pytest.mark.asyncio
    async def test_analyze_message_for_tasks_order_inquiry(
        self, task_integration, sample_message_data
    ):
        ...
        inquiry_message["message"][
            "text"
        ] = "Когда будет готов мой заказ? Статус заказа пожалуйста."
    ...
    @pytest.mark.asyncio
    async def test_analyze_message_for_tasks_price_negotiation(
        self, task_integration, sample_message_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_analyze_message_for_tasks_quality_issue(
        self, task_integration, sample_message_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_analyze_message_for_tasks_return_request(
        self, task_integration, sample_message_data
    ):
        ...
        return_message["message"][
            "text"
        ] = "Хочу вернуть товар, он не подошел. Обмен или возврат."
    ...
    @pytest.mark.asyncio
    async def test_analyze_message_for_tasks_custom_order(
        self, task_integration, sample_message_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_analyze_message_for_vip_client_attention(
        self, task_integration, sample_message_data, sample_customer
    ):
        ...
        task_integration.db.query.return_value.filter.return_value.count.return_value = (
            15
        )

        triggers = await task_integration._analyze_message_for_tasks(
            "Простой вопрос о заказе",
            sample_customer,
            sample_message_data.get("item_id"),
        )

        # Должен быть триггер для VIP клиента
        vip_trigger = next(
            (t for t in triggers if t["type"] == "vip_client_attention"), None
        )
    ...
    @pytest.mark.asyncio
    async def test_create_task_from_trigger_success(
        self, task_integration, mock_task_service, sample_customer
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_task_from_trigger_no_assignee(
        self, task_integration, mock_task_service, sample_customer
    ):
    ...
    @pytest.mark.asyncio
    async def test_determine_task_assignee_priority_logic(
        self, task_integration, db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_determine_task_assignee_vip_client(
        self, task_integration, db_session, sample_customer
    ):
    ...
    @pytest.mark.asyncio
    async def test_analyze_and_create_tasks_full_flow(
        self, task_integration, sample_message_data, sample_customer, mock_task_service
    ):
        """Тест полного потока анализа и создания задач"""
        # Мокаем анализ сообщения
        task_integration._analyze_message_for_tasks = AsyncMock(
            return_value=[
                {
                    "type": "order_inquiry",
                    "title": "Запрос информации о заказе",
    ...
    @pytest.mark.asyncio
    async def test_analyze_and_create_tasks_no_triggers(
        self, task_integration, sample_message_data, sample_customer
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_followup_task_success(
        self, task_integration, mock_task_service, sample_customer
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_order_tracking_task_success(
        self, task_integration, mock_task_service, sample_customer
    ):
    ...
    @pytest.mark.asyncio
    async def test_update_task_status_from_avito_success(
        self, task_integration, mock_task_service
    ):
