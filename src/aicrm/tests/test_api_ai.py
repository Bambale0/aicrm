...
class TestAIApi:
    ...
    async def test_analyze_intent_success(self, async_client: AsyncClient):
        ...
        login_data = {"email": "test_ai@example.com", "password": "testpass123"}
        ...
        # Мокаем AI сервис
        with patch(
            "src.aicrm.services.ai.intent_service.AIIntentService"
        ) as mock_service_class:
            mock_service = MagicMock()
            mock_service.analyze_intent.return_value = "order"
            mock_service.generate_response.return_value = (
                "Отлично! Я помогу вам оформить заказ на футболку с логотипом."
            )
            mock_service._get_suggested_actions.return_value = [
                "create_order",
                "ask_design_details",
            ]
            ...
            with patch(
                "src.aicrm.services.ai_usage_service.AIUsageService"
            ) as mock_usage_class:
...
class TestAIRouterUnit:
    ...
    def test_get_available_models_unit(self):
        ...
        with patch(
            "src.aicrm.api.routers.ai.OPENROUTER_MODELS",
    ...
    def test_get_ai_status_unit(self):
        ...
        with (
            patch("src.aicrm.api.routers.ai.UnifiedAIClient", return_value=mock_client),
    ...
    def test_get_monthly_usage_unit(self):
        ...
        with patch(
            "src.aicrm.api.routers.ai.AIUsageService", return_value=mock_usage_service
        ):
    ...
    def test_update_ai_settings_invalid_max_tokens(self):
        ...
        assert exc_info.value.status_code == 400
        assert (
            "Максимальное количество токенов должно быть от 1 до 8000"
