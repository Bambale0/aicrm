...
from aicrm.models.user import User, get_password_hash
from aicrm.models.customer import Customer
from aicrm.models.order import Order
from aicrm.services.auth import create_access_token
...
class TestCustomerManagement:
    ...
    @pytest.mark.asyncio
    async def test_create_and_retrieve_customer(self, client, auth_headers, test_db_session):
    ...
    async def test_customer_search(self, client, auth_headers):
        ...
        response = await client.get("/customers/search/?query=john", headers=auth_headers)
        assert response.status_code == 200
...
class TestOrderManagement:
    ...
    @pytest.mark.asyncio
    async def test_create_order_with_production_workflow(self, client, auth_headers, test_db_session):
        ...
        customer_response = await client.post("/customers/", json=customer_data, headers=auth_headers)
        ...
        response = await client.get(f"/orders/{order_id}/production-progress", headers=auth_headers)
        assert response.status_code == 200
    ...
    async def test_production_step_management(self, client, auth_headers):
        ...
        order_response = await client.post("/orders/", json=order_data, headers=auth_headers)
...
class TestErrorHandling:
    ...
    async def test_validation_errors(self, client, auth_headers):
        ...
        response = await client.post("/customers/", json=invalid_customer, headers=auth_headers)
        ...
        response = await client.post("/orders/", json=incomplete_order, headers=auth_headers)
        assert response.status_code == 422  # Validation error
