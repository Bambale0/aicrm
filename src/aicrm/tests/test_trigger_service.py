...
class TestTriggerService:
    ...
    async def test_handle_trigger_event_with_triggers(self, trigger_service, mock_db):
        ...
        with (
            patch.object(
                trigger_service, "_check_trigger_conditions", new_callable=AsyncMock
            ) as mock_check,
            patch.object(
                trigger_service, "_move_to_target_stage", new_callable=AsyncMock
            ) as mock_move,
        ):
    ...
    @pytest.mark.asyncio
    async def test_handle_trigger_event_conditions_not_met(
        self, trigger_service, mock_db
    ):
    ...
    @pytest.mark.asyncio
    async def test_check_trigger_conditions_no_conditions(
        self, trigger_service, mock_db
    ):
    ...
    @pytest.mark.asyncio
    async def test_check_trigger_conditions_with_conditions(
        self, trigger_service, mock_db
    ):
        ...
        with patch.object(
            trigger_service, "_get_entity_data", new_callable=AsyncMock
        ) as mock_get_data:
            mock_get_data.return_value = {"status": "active"}

            result = await trigger_service._check_trigger_conditions(
                mock_trigger, 1, {}
            )
    ...
    @pytest.mark.asyncio
    async def test_check_trigger_conditions_entity_not_found(
        self, trigger_service, mock_db
    ):
    ...
    def test_evaluate_conditions_simple(self, trigger_service):
        ...
        result = trigger_service._evaluate_conditions(
            conditions, entity_data, event_data
        )
    ...
    def test_check_condition_not_equals(self, trigger_service):
        ...
        assert (
            trigger_service._check_condition("active", "not_equals", "inactive") is True
        )
        assert (
            trigger_service._check_condition("active", "not_equals", "active") is False
        )
    ...
    def test_check_condition_contains(self, trigger_service):
        ...
        assert (
            trigger_service._check_condition("hello world", "contains", "world") is True
        )
        assert (
            trigger_service._check_condition("hello world", "contains", "foo") is False
        )
    ...
    async def test_move_to_target_stage_customer(self, trigger_service, mock_db):
        ...
        with patch.object(
            trigger_service, "_move_customer_to_stage", new_callable=AsyncMock
        ) as mock_move:
    ...
    @pytest.mark.asyncio
    async def test_move_to_target_stage_unsupported_entity(
        self, trigger_service, mock_db
    ):
