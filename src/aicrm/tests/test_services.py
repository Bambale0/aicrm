...
class TestSessionService:
    ...
    async def test_create_session(self, session_service):
        ...
        user_data = {"id": 1, "email": "test@example.com", "ip_address": "127.0.0.1"}
...
class TestWorkflowEngine:
    ...
    async def test_workflow_execution(self, workflow_engine):
        ...
        with (
            patch.object(
                workflow_engine, "_validate_order_data", new_callable=AsyncMock
            ) as mock_validate,
            patch.object(
                workflow_engine, "_create_production_steps", new_callable=AsyncMock
            ) as mock_create,
            patch.object(
                workflow_engine, "_assign_production_tasks", new_callable=AsyncMock
            ) as mock_assign,
            patch.object(
                workflow_engine,
                "_notify_customer_order_created",
