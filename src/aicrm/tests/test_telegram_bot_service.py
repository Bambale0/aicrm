...
class TestTelegramBotService:
    ...
    async def test_initialize_bot_success(self, bot_service, mock_db):
        ...
        with (
            patch("src.aicrm.services.telegram_bot_service.settings") as mock_settings,
    ...
    async def test_start_polling_success(self, bot_service, mock_db):
        ...
        with (
            patch.object(mock_app, "initialize", new_callable=AsyncMock),
    ...
    async def test_stop_bot(self, bot_service, mock_db):
        ...
        with (
            patch.object(mock_app, "stop", new_callable=AsyncMock),
    ...
    async def test_handle_start_command(self, bot_service, mock_db):
        ...
        with (
            patch.object(
                bot_service, "_get_or_create_chat", new_callable=AsyncMock
            ) as mock_get_chat,
            patch.object(
                bot_service, "_log_communication", new_callable=AsyncMock
            ) as mock_log,
        ):
    ...
    async def test_handle_message(self, bot_service, mock_db):
        ...
        with (
            ...
            patch.object(
                bot_service, "_process_with_ai", new_callable=AsyncMock
            ) as mock_process_ai,
            patch.object(
                bot_service, "_log_communication", new_callable=AsyncMock
            ) as mock_log,
            patch.object(
                bot_service, "_handle_ai_actions", new_callable=AsyncMock
    ...
    async def test_process_with_ai(self, bot_service, mock_db):
        ...
        with (
            patch.object(
                bot_service.ai_service,
                "process_customer_message",
                new_callable=AsyncMock,
            ) as mock_ai,
            patch.object(
                bot_service, "_get_customer_context", new_callable=AsyncMock
            ) as mock_context,
        ):
    ...
    async def test_get_or_create_chat_new(self, bot_service, mock_db):
        ...
        with (
            patch(
                "src.aicrm.services.telegram_bot_service.TelegramChat",
                return_value=mock_telegram_chat,
            ),
            patch.object(
                bot_service, "_find_customer_by_telegram_data", new_callable=AsyncMock
    ...
    async def test_log_communication(self, bot_service, mock_db):
        ...
        with (
            patch.object(
                bot_service, "_get_chat_by_id", new_callable=AsyncMock
            ) as mock_get_chat,
            patch(
                "src.aicrm.services.telegram_bot_service.Communication"
    ...
    async def test_send_message_to_chat_success(self, bot_service, mock_db):
        ...
        assert result is True
        bot_service.bot.send_message.assert_called_once_with(
    ...
    async def test_handle_ai_actions_order_creation(self, bot_service, mock_db):
        ...
        with patch.object(
            bot_service, "_process_order_creation", new_callable=AsyncMock
        ) as mock_process_order:
            mock_process_order.return_value = None

            await bot_service._handle_ai_actions(
                ai_result, mock_telegram_chat, mock_update
            )
    ...
    async def test_handle_ai_actions_complaint(self, bot_service, mock_db):
        ...
        with patch.object(
            bot_service, "_escalate_to_support", new_callable=AsyncMock
        ) as mock_escalate:
    ...
    async def test_process_order_creation_success(self, bot_service, mock_db):
        ...
        with (
            ...
            await bot_service._process_order_creation(
                mock_telegram_chat, message_text, mock_update
            )
    ...
    async def test_process_order_creation_failure(self, bot_service, mock_db):
        ...
        with patch.object(
            bot_service.order_service, "process_order_request", new_callable=AsyncMock
        ) as mock_process:
