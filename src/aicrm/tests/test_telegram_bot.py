...
class TestTelegramBotService:
    ...
    def sample_customer(self):
        ...
        return Customer(
            id=1, name="Test Customer", email="test@example.com", total_orders=5
        )
    ...
    @pytest.mark.asyncio
    async def test_get_or_create_chat_existing(
        self, bot_service, db_session, sample_chat
    ):
    ...
    def test_get_bot_stats(self, bot_service, db_session):
        ...
        # Мокаем запросы к БД
        db_session.query.return_value.count.side_effect = [
            10,
            7,
            150,
...
class TestTelegramOrderService:
    ...
    def sample_customer(self):
        """Фикстура для тестового клиента"""
        return Customer(id=1, name="Test Customer", email="test@example.com")

    @pytest.fixture
    def sample_chat(self, sample_customer):
        """Фикстура для тестового чата"""
        return TelegramChat(id=1, chat_id="123456789", customer_id=sample_customer.id)

    @pytest.mark.asyncio
    async def test_process_order_request_success(
        self, order_service, db_session, sample_chat, sample_customer
    ):
        ...
        # Мокаем AI сервис
        order_service.ai_service.ai_client.chat_completion = AsyncMock(
            return_value='{"has_order_intent": true, "order_data": {"product_type": "футболка", "quantity": 5, "size": "M", "color": "черный"}, "confidence": 0.9, "missing_info": []}'
        )
        ...
        order_service._generate_order_confirmation_message = Mock(
            return_value="Заказ создан!"
        )
    ...
    @pytest.mark.asyncio
    async def test_process_order_request_no_order_intent(
        self, order_service, db_session, sample_chat, sample_customer
    ):
        ...
        order_service.ai_service.ai_client.chat_completion = AsyncMock(
            return_value='{"has_order_intent": false, "order_data": {}, "confidence": 0, "missing_info": []}'
        )
    ...
    @pytest.mark.asyncio
    async def test_get_customer_orders_summary(
        self, order_service, db_session, sample_chat, sample_customer
    ):
        ...
        mock_orders = [
            Mock(
                id=1,
                status=Mock(display="Ожидает обработки"),
                total_amount=500,
                created_at=Mock(strftime=Mock(return_value="01.01.2024")),
            ),
            Mock(
                id=2,
                status=Mock(display="Готов"),
                total_amount=1200,
                created_at=Mock(strftime=Mock(return_value="02.01.2024")),
            ),
        ]
    ...
    @pytest.mark.asyncio
    async def test_get_customer_orders_summary_no_customer(
        self, order_service, sample_chat
    ):
