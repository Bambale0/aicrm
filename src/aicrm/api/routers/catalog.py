...
async def get_categories(
    type_filter: Optional[str] = Query(
        None, description="Тип категории: 'product' или 'service'"
    ),
    ...
    return (
        query.filter(Category.is_active.is_(True)).order_by(Category.sort_order).all()
    )
...
async def get_product(product_id: int, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "avg_rating": product.avg_rating,
        "tags": (
            [tag.tag for tag in getattr(product, "tags", [])]
...
async def get_service(service_id: int, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "avg_rating": service.avg_rating,
        "tags": (
            [tag.tag for tag in getattr(service, "tags", [])]
            if getattr(service, "tags", [])
            else []
        ),
        "created_at": service.created_at,
...
async def search_catalog(
    ...
    if not type_filter or type_filter == "product":
        ...
        products_result = (
            product_query.filter(Product.is_active.is_(True)).limit(limit).all()
        )
    ...
    if not type_filter or type_filter == "service":
        ...
        services_result = (
            service_query.filter(Service.is_active.is_(True)).limit(limit).all()
        )
