...
class SystemSettingsUpdate(BaseModel):
    ...
    session_timeout_minutes: Optional[int] = Field(
        None, gt=0, description="Таймаут сессии в минутах"
    )
    max_login_attempts: Optional[int] = Field(
        None, gt=0, description="Максимум попыток входа"
    )
    lockout_duration_minutes: Optional[int] = Field(
        None, gt=0, description="Длительность блокировки"
    )
    password_min_length: Optional[int] = Field(
        None, gt=0, description="Минимальная длина пароля"
    )
    ...
    monitoring_interval_seconds: Optional[int] = Field(
        None, gt=0, description="Интервал мониторинга"
    )
    alert_email_enabled: Optional[bool] = Field(
        None, description="Включить email оповещения"
    )
    performance_thresholds: Optional[Dict[str, Any]] = Field(
        None, description="Пороги производительности"
    )

    log_level: Optional[str] = Field(None, description="Уровень логирования")
    log_retention_days: Optional[int] = Field(
        None, gt=0, description="Срок хранения логов"
    )
    ...
    notifications_enabled: Optional[bool] = Field(
        None, description="Включить уведомления"
    )
    email_notifications_enabled: Optional[bool] = Field(
        None, description="Email уведомления"
    )
    websocket_notifications_enabled: Optional[bool] = Field(
        None, description="WebSocket уведомления"
    )
    push_notifications_enabled: Optional[bool] = Field(
        None, description="Push уведомления"
    )

    auto_backup_enabled: Optional[bool] = Field(None, description="Автобэкап")
    backup_frequency_hours: Optional[int] = Field(
        None, gt=0, description="Частота бэкапов"
    )
    backup_retention_days: Optional[int] = Field(
        None, gt=0, description="Срок хранения бэкапов"
    )
    ...
    update_check_frequency_hours: Optional[int] = Field(
        None, gt=0, description="Частота проверки обновлений"
    )
...
class TelegramSettingsUpdate(BaseModel):
    ...
    telegram_notification_enabled: Optional[bool] = Field(
        None, description="Включить уведомления"
    )
