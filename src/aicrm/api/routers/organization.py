...
async def register_organization(
    org_data: OrganizationCreate, db: Session = Depends(get_master_db)
):
    """
    ...
    try:
        ...
        logger.info(
            f"Organization registered successfully: {organization.name} (ID: {organization.id})"
        )
...
async def check_slug_availability(slug: str, db: Session = Depends(get_master_db)):
    """
    ...
    """
    exists = (
        db.query(Organization).filter(Organization.slug == slug).first() is not None
    )
...
async def list_organizations(
    ...
    return [
        OrganizationAdminResponse.model_validate(org.__dict__) for org in organizations
    ]
...
async def update_organization(
    ...
    if updates.email and updates.email != organization.email:
        existing = (
            db.query(Organization).filter(Organization.email == updates.email).first()
        )
        if existing:
            raise HTTPException(
...
async def deactivate_organization(
    ...
    logger.info(
        f"Organization deactivated: {organization.name} (ID: {organization.id})"
    )
...
async def delete_organization(
    ...
    try:
        ...
        logger.warning(
            f"Organization deleted: {organization.name} (ID: {organization.id})"
        )
...
async def get_organizations_stats(
    ...
    plans_stats = (
        db.query(Organization.plan, db.func.count(Organization.id))
