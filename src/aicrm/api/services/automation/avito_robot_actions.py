...
from ...models.automation import EntityType, RobotActionConfig
...
class AvitoRobotActions:
    ...
    async def _execute_generate_ai_response(
        ...
        result = await avito_service.generate_auto_response(
            chat_id=chat_id, message_text=message_text, context=context
        )
    ...
    async def _execute_send_standard_response(
        ...
        result = await avito_service.send_standard_response(
            chat_id=chat_id, response_type=response_type, custom_data=custom_data
        )
    ...
    async def _execute_escalate_complex_query(
        ...
        return {
            "status": (
                "query_escalated" if result.get("success") else "escalation_failed"
            ),
            "chat_id": chat_id,
            "escalation_reason": escalation_reason,
