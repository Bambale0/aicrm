...
class PluginSandboxExecutor:
        ...
        try:
            ...
            for limit_type, limit_value in original_limits.items():
                try:
                    ...
                    logger.warning(
                        f"Failed to restore resource limit {limit_type}: {e}"
                    )
            ...
            with self._resource_limits():
                ...
                if not plugin_instance:
                    raise SandboxSecurityError(
                        f"Plugin instance with action '{action_name}' not found"
                    )

                # Execute action with timeout
                result = await self._execute_with_timeout(
                    plugin_instance, action_name, parameters, context
                )
    ...
    async def _execute_with_timeout(
        ...
        try:
            ...
            if not action_method or not callable(action_method):
                raise SandboxSecurityError(
                    f"Action method '{action_name}' not found or not callable"
                )
            ...
            if asyncio.iscoroutinefunction(action_method):
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                    result = await loop.run_in_executor(
                        executor, functools.partial(action_method, parameters, context)
                    )
    ...
    def _check_security(self, compiled_code):
        ...
        for instruction in bytecode:
            ...
            if opname == "LOAD_CONST" and instruction.argval in dangerous_imports:
                ...
                raise SandboxSecurityError(
                    f"Dangerous import detected: {instruction.argval}"
                )
    ...
    async def validate_plugin_code(self, plugin_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"valid": True, "message": "Plugin code is valid"}
            ...
            return {"valid": False, "error": str(e), "error_type": "security"}
