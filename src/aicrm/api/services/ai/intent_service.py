...
class AIIntentService:
    ...
    def _load_intent_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            IntentType.STATUS_CHECK: [
                r"статус\s+заказа",
                r"где\s+мой\s+заказ",
                r"когда\s+будет\s+готово",
                r"проверить\s+заказ",
                r"отследить\s+заказ",
            ],
            IntentType.MODIFICATION: [
                r"изменить\s+заказ",
                r"добавить\s+в\s+заказ",
                r"убрать\s+из\s+заказа",
                r"исправить\s+заказ",
                r"корректировка",
            ],
            IntentType.CANCELLATION: [
                r"отменить\s+заказ",
                r"отказаться\s+от\s+заказа",
                r"вернуть\s+деньги",
            ],
        }
    ...
    async def _refine_intent_with_context(
        self, initial_intent: str, text: str, context: Dict[str, Any]
    ) -> str:
        ...
        if "conversation_history" in context:
            ...
            if history:
                ...
                if (
                    last_intent == IntentType.PRICE_INQUIRY
                    and initial_intent == IntentType.GENERAL
                ):
    ...
    async def generate_response(
        self, intent: str, original_text: str, context: Optional[Dict[str, Any]] = None
    ) -> str:
        """
        ...
        try:
            ...
            response = await self._validate_and_improve_response(
            ...
            try:
                ...
                metrics_service.record_ai_request(
                    "response_generation", "local", "success"
                )
            except ImportError:
                pass  # Метрики не обязательны

            logger.info(
                "Response generated", intent=intent, response_length=len(response)
            )
            ...
            try:
                ...
                metrics_service.record_ai_request(
                    "response_generation", "local", "error"
                )
            except ImportError:
                pass  # Метрики не обязательны
...
Если вопрос срочный, позвоните: +7 (495) 123-45-67
    ...
    def _prepare_response_data(
        self, intent: str, original_text: str, context: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        if context and "user_info" in context:
            ...
            if user_info.get("total_orders", 0) > 0:
                data["loyalty_info"] = (
                    "Как постоянному клиенту, вам доступны специальные условия."
                )
...
{discount_text}
    ...
    def _format_service_prices(self) -> str:
        ...
        for service_key, service_data in services.items():
            prices.append(
                f"• {service_data['name']}: от {service_data['base_price']} руб/шт"
            )
...
Общий срок: от 3 дней
    ...
    async def _validate_and_improve_response(
        self, response: str, intent: str, context: Optional[Dict[str, Any]]
    ) -> str:
