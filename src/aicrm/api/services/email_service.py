...
class EmailService:
    ...
    async def send_email(self, message: EmailMessage) -> bool:
        ...
        try:
            ...
            success = await loop.run_in_executor(None, self._send_smtp, msg, to_list)
    ...
    def _send_smtp(self, msg: MIMEMultipart, to_list: List[str]) -> bool:
        ...
        try:
            ...
            server.sendmail(
                self.from_email or self.smtp_username or "", to_list, msg.as_string()
            )
    ...
    async def send_order_confirmation(
        self, order_id: int, customer_email: str, customer_name: str
    ) -> bool:
        """
    ...
    async def send_task_notification(
        self, task_id: int, assignee_email: str, assignee_name: str
    ) -> bool:
        """
    ...
    async def get_emails(
        ...
        return await loop.run_in_executor(
            None, self._get_emails_imap, folder, limit, offset, search_criteria
        )

    def _get_emails_imap(
        self, folder: str, limit: int, offset: int, search_criteria: Optional[str]
    ) -> List[ReceivedEmailMessage]:
            ...
            for msg_id in ids_to_fetch:
                try:
                    ...
                    email_message = self._parse_email_message(
                        msg_data[0], msg_id.decode("utf-8"), folder
                    )
                    if email_message:
    ...
    def _parse_email_message(
        self, raw_data: bytes, msg_id: str, folder: str
    ) -> Optional[ReceivedEmailMessage]:
    ...
    def _extract_body_and_attachments(
        self, msg
    ) -> Tuple[str, Optional[str], List[EmailAttachment]]:
    ...
    async def mark_as_read(self, message_ids: List[str], folder: str = "INBOX") -> bool:
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self._mark_messages_imap, message_ids, folder, "+FLAGS", "\\Seen"
        )

    async def mark_as_unread(
        self, message_ids: List[str], folder: str = "INBOX"
    ) -> bool:
    ...
    async def flag_messages(
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self._mark_messages_imap, message_ids, folder, "+FLAGS", "\\Flagged"
        )
    ...
    async def delete_messages(
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self._mark_messages_imap, message_ids, folder, "+FLAGS", "\\Deleted"
        )

    def _mark_messages_imap(
        self, message_ids: List[str], folder: str, action: str, flag: str
    ) -> bool:
    ...
    async def move_messages(
        self, message_ids: List[str], from_folder: str, to_folder: str
    ) -> bool:
        """Переместить сообщения между папками"""
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self._move_messages_imap, message_ids, from_folder, to_folder
        )

    def _move_messages_imap(
        self, message_ids: List[str], from_folder: str, to_folder: str
    ) -> bool:
    ...
    async def add_contact(self, email: str, name: str) -> bool:
        ...
        try:
            contact = EmailContact(
                email=email.lower(), name=name, last_contact=datetime.now()
            )
    ...
    async def get_contacts(self, search: Optional[str] = None) -> List[EmailContact]:
        ...
        return sorted(
            contacts, key=lambda c: c.last_contact or datetime.min, reverse=True
        )
    ...
    async def search_emails(
        ...
        return await self.get_emails(
            folder=folder, limit=limit, search_criteria=imap_criteria
        )
