...
class Workflow:
    ...
    async def execute(self, initial_context: Dict[str, Any]) -> bool:
        ...
        metrics_service.record_ai_request(
            "workflow_execution", "local", "success" if success else "error"
        )
...
class WorkflowEngine:
    ...
    def _register_standard_workflows(self):
        ...
        payment_workflow = Workflow(
            "payment_processing", "Обработка платежа", EntityType.ORDER
        )
        payment_workflow.add_step(
            WorkflowStep(
                "validate_payment",
                "Валидация платежа",
        ...
        completion_workflow = Workflow(
            "production_completion", "Завершение производства", EntityType.ORDER
        )
        completion_workflow.add_step(
            WorkflowStep(
        ...
        complaint_workflow = Workflow(
            "complaint_handling", "Обработка жалоб", EntityType.CUSTOMER
        )
        complaint_workflow.add_step(
            WorkflowStep(
                "analyze_complaint",
                "Анализ жалобы",
    ...
    async def trigger_workflow(
        ...
        execution_workflow = Workflow(
            execution_id, f"{workflow.name} (Execution {execution_id})", entity_type
        )

        # Копируем шаги
        for step in workflow.steps:
            execution_workflow.add_step(
                WorkflowStep(
                    step.step_id, step.name, step.action, step.conditions.copy()
                )
            )
    ...
    async def _execute_workflow_async(
        ...
        try:
            ...
            logger.error(
                f"Workflow execution failed: {workflow.workflow_id}", error=str(e)
            )
        finally:
    ...
    async def _create_production_steps(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for i, step_name in enumerate(steps_config, 1):
            step = ProductionStep(
                order_id=order_id, name=step_name, status=StepStatus.pending, sequence=i
            )
            self.db.add(step)
    ...
    async def _notify_customer_order_created(
        self, context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _validate_payment(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if payment_amount < order_total:
            raise ValueError(
                f"Payment amount {payment_amount} less than order total {order_total}"
            )
    ...
    async def _start_production_workflow(
        self, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Запуск производства"""
        order_id = context["entity_id"]
    ...
    async def _check_production_completion(
        self, context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _notify_customer_delivery_ready(
        self, context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _analyze_complaint(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        severity = "low"
        if any(
            word in complaint_text.lower()
            for word in ["брак", "плохо", "ужасно", "возврат"]
        ):
            severity = "high"
        elif any(
            word in complaint_text.lower() for word in ["задержка", "долго", "проблема"]
        ):
            severity = "medium"
