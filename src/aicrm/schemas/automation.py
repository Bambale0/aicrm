...
class ProcessBase(BaseModel):
    ...
    name: str = Field(
        ..., description="Название процесса", example="Продажа полиграфии"
    )
...
class TriggerBase(BaseModel):
    ...
    conditions: Optional[Dict[str, Any]] = Field(
        None, description="Условия срабатывания"
    )
    target_stage_id: int = Field(..., description="ID целевой стадии")

    model_config = ConfigDict(
        use_enum_values=True
    )  # Использовать строковые значения enum вместо самих enum объектов
...
class AutomationEventRequest(BaseModel):
    ...
    event_data: Optional[Dict[str, Any]] = Field(
        None, description="Дополнительные данные события"
    )
...
class AutomationChainRequest(BaseModel):
    ...
    entity_type: EntityType = Field(
        ..., description="Тип сущности для автоматизации", example="customer"
    )
...
class AutomationChainResponse(BaseModel):
    ...
    message: str = Field(..., description="Сообщение о результате")
    generated_process: Optional[GeneratedProcess] = Field(
        None, description="Сгенерированный процесс"
    )
    applied_changes: Optional[Dict[str, Any]] = Field(
        None, description="Примененные изменения"
    )
...
class OptimizationResponse(BaseModel):
    ...
    message: str = Field(..., description="Сообщение о результате")
    optimizations_applied: List[Dict[str, Any]] = Field(
        ..., description="Примененные оптимизации"
    )
    performance_improvements: Optional[Dict[str, Any]] = Field(
        None, description="Улучшения производительности"
    )
...
class ImprovementSuggestion(BaseModel):
    ...
    type: str = Field(
        ..., description="Тип предложения: bottleneck, optimization, new_scenario"
    )
    ...
    implementation_complexity: str = Field(
        ..., description="Сложность реализации: low, medium, high"
    )
