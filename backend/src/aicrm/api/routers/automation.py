...
async def get_automation_logs(
    skip: int = Query(0, ge=0),
    limit: int = Query(100, ge=1, le=1000),
    level: Optional[str] = Query(None, description="Уровень логирования: error, warning, success, info"),
    ...
    try:
        ...
        if date_from:
            query = query.filter(AutomationLog.timestamp >= datetime.fromisoformat(date_from.replace('Z', '+00:00')))
        if date_to:
            query = query.filter(AutomationLog.timestamp <= datetime.fromisoformat(date_to.replace('Z', '+00:00')))
