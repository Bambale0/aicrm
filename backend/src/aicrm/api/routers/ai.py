...
async def campaign_analyze_intent(
    ...
    try:
        ...
        response = await ai_service.generate_response(intent, request.message, enhanced_context)

        # Логируем использование токенов (привязываем к кампании)
        model_used = campaign_ai_settings.default_model if campaign_ai_settings else "deepseek/deepseek-chat-v3.1"
    ...
    try:
        ...
        if campaign.ai_settings:
            ...
            campaign_client = UnifiedAIClient(
                ...
                api_key=getattr(campaign.ai_settings, f"{campaign.ai_settings.provider}_api_key"),
...
""".strip()
        ...
        response = await campaign_client.chat_completion(
            ...
            model=request.model,
            temperature=request.temperature or (campaign.ai_settings.temperature if campaign.ai_settings else 0.7),
            max_tokens=request.max_tokens or (campaign.ai_settings.max_tokens if campaign.ai_settings else None)
        )

        # Логируем использование токенов
        model_used = request.model or (campaign.ai_settings.default_model if campaign.ai_settings else campaign_client.provider.value)
