...
class WebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, room: str = "general", user_id: int = None):
    ...
    def disconnect(self, websocket: WebSocket, room: str = "general", user_id: int = None):
...
async def chat_websocket(
    ...
    if not await rate_limiter.check_rate_limit(connection_key, limit=5, window_seconds=300):
    ...
    try:
        while True:
            ...
            if not await rate_limiter.check_rate_limit(message_key, limit=20, window_seconds=60):
...
async def notifications_websocket(
    ...
    try:
        while True:
            ...
            if data == "ping":
                if await rate_limiter.check_rate_limit(heartbeat_key, limit=10, window_seconds=60):
        ...
        logger.info(f"User {user.id} disconnected from notifications for user {target_user_id}")
...
async def global_notifications_websocket(
    ...
    if not await rate_limiter.check_rate_limit(global_key, limit=2, window_seconds=600):  # 2 подключения за 10 минут
        await websocket.close(code=status.HTTP_429_TOO_MANY_REQUESTS)
        return
    ...
    try:
        while True:
            ...
            if data == "ping":
                if await rate_limiter.check_rate_limit(global_heartbeat_key, limit=5, window_seconds=60):
                    await websocket.send_json({"type": "pong"})
                else:
                    await websocket.send_json({
                        "type": "error",
...
async def broadcast_notification(
    ...
    return {"status": "broadcast", "recipients": sum(len(conns) for conns in ws_manager.active_connections.values())}
