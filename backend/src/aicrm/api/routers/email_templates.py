...
async def get_templates(
    ...
    limit: int = Query(50, description="Максимальное количество результатов", ge=1, le=100),
...
async def update_template(
    ...
    try:
        service = get_email_template_service(db)
        template = await service.update_template(template_id, update_data, current_user.id)
        return {
            "success": True,
            "template": template.to_dict(),
...
async def duplicate_template(
    ...
    try:
        ...
        if not new_name or not new_display_name:
            raise HTTPException(
                status_code=400,
                detail="Необходимо указать new_name и new_display_name"
            )
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка дублирования шаблона: {str(e)}")
...
async def get_templates_by_category(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка получения шаблонов категории: {str(e)}")
...
async def get_default_template(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка получения шаблона по умолчанию: {str(e)}")
...
async def initialize_default_templates(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка инициализации шаблонов: {str(e)}")


@router.get("/export/all", response_model=Dict[str, Any])
async def export_templates(
    category: Optional[str] = Query(None, description="Экспорт только указанной категории"),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка экспорта шаблонов: {str(e)}")


@router.post("/import", response_model=Dict[str, Any])
async def import_templates(
    import_data: Dict[str, Any],
    overwrite_existing: bool = Query(False, description="Перезаписывать существующие шаблоны"),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        if not templates_data:
            raise HTTPException(status_code=400, detail="Не указаны шаблоны для импорта")

        service = get_email_template_service(db)
        result = await service.import_templates(templates_data, current_user.id, overwrite_existing)
...
async def get_template_variables(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка получения переменных: {str(e)}")
...
async def validate_template_variables(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка валидации переменных: {str(e)}")
