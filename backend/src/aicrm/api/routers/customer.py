...
async def create_customer(
    ...
    customer = await cached_customer_service.create_customer_cached(db, customer_data.dict())
    return customer
...
async def update_customer(
    ...
    customer = await cached_customer_service.update_customer_cached(db, customer_id, customer_data.dict(exclude_unset=True))
    if not customer:
        raise HTTPException(status_code=404, detail="Customer not found")
    return customer
