...
async def save_telegram_settings(
    ...
    allowed_fields = {
        ...
        'notification_email', 'sync_interval', 'max_concurrent_chats'
    }
...
async def test_telegram_connection(
    ...
    try:
        ...
        return {
            "success": False,
            "message": "Таймаут подключения к Telegram API"
        }
    except Exception as e:
        return {
            "success": False,
...
async def test_telegram_webhook(
    ...
    if not settings or not settings.webhook_url:
        raise HTTPException(
            status_code=400,
            detail="Webhook URL не настроен"
        )
    ...
    try:
        ...
        return {
            "success": False,
            "message": "Таймаут ожидания ответа от webhook"
        }
    except Exception as e:
        return {
            "success": False,
