...
async def get_chats(user_id: int, limit: int = 50, offset: int = 0, db: Session = Depends(get_db)):
    ...
    try:
        ...
        chats = query.order_by(
            AvitoChatSettings.last_message_at.desc().nulls_last()
        ...
        for chat in chats:
            ...
            customer_email = chat.customer.email if hasattr(chat, 'customer') and chat.customer else None
...
async def get_messenger_stats(db: Session = Depends(get_db)):
    ...
    try:
        ...
        ai_enabled_chats = db.query(AvitoChatSettings).filter(
            AvitoChatSettings.ai_enabled.is_(True)
        ).count()
    ...
    try:
        ...
        async with AvitoService() as avito_service:
            try:
                ...
                logger.info(f"Успешное тестирование Avito API: получено {len(items)} объявлений")
                ...
                return {
                    "success": False,
                    "message": f"Ошибка API Avito: {str(e)}"
                }
            except Exception as api_error:
                logger.error(f"Неожиданная ошибка при тестировании Avito API: {api_error}")
                return {
                    "success": False,
...
async def link_avito_chat_to_customer(
    ...
    try:
        ...
        chat = db.query(AvitoChatSettings).filter(AvitoChatSettings.chat_id == chat_id).first()
        if not chat:
            raise HTTPException(status_code=404, detail="Чат не найден")
...
async def unlink_avito_chat_from_customer(
    chat_id: str,
    db: Session = Depends(get_db)
):
    """
