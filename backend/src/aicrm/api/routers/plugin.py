...
from ...schemas.plugin import (
    PluginResponse, PluginCreate, PluginUpdate,
    PluginActionRequest, PluginActionResponse,
    PluginRegistryResponse, PluginInstallRequest,
    PluginSandboxTest, PluginSandboxResponse
)
...
async def get_plugin_registry(
    ...
    for plugin in registry_plugins:
        result.append(PluginRegistryResponse(
            name=plugin['name'],
            display_name=plugin['display_name'],
            description=plugin['description'],
            version=plugin['version'],
...
async def get_plugin_actions(
    ...
    if plugin_name not in plugin_manager.plugin_actions:
        raise HTTPException(status_code=404, detail="Plugin not found or has no actions")
...
async def get_plugin_hooks(
    ...
    from ...models.plugin import PluginHook, Plugin
...
async def get_plugin_logs(
    ...
    from ...models.plugin import PluginAction, Plugin

    plugin = db.query(Plugin).filter(Plugin.name == plugin_name).first()
    if not plugin:
        raise HTTPException(status_code=404, detail="Plugin not found")

    logs = db.query(PluginAction).filter(
        PluginAction.plugin_id == plugin.id
    ).order_by(PluginAction.created_at.desc()).limit(limit).all()
