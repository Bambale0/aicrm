...
async def create_campaign(
    ...
    try:
        ...
        ai_settings = CampaignAISettings(
            campaign_id=campaign.id
        )
        db.add(ai_settings)
        db.commit()

        logger.info(f"Campaign created: ID {campaign.id}, name '{campaign.name}' by user {current_user.id}")
...
async def update_campaign_ai_settings(
    ...
    try:
        ...
        logger.info(f"Campaign AI settings updated: campaign_id {campaign_id} by user {current_user.id}")
