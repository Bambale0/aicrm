...
class CampaignAISettingsCreate(BaseModel):
    ...
    max_tokens: Optional[int] = Field(None, description="Максимальное количество токенов", ge=1, le=8000)
    ...
    auto_reply_temperature: Optional[float] = Field(None, description="Температура автоответов", ge=0.0, le=2.0)
    auto_reply_max_tokens: Optional[int] = Field(None, description="Максимальные токены автоответов", ge=1, le=8000)

    # Специфические настройки
    custom_prompt: Optional[str] = Field(None, description="Кастомный промпт для кампании")
    target_audience: Optional[str] = Field(None, description="Целевая аудитория", max_length=255)
    brand_voice: Optional[str] = Field(None, description="Стиль общения", max_length=100)

    # Лимиты
    daily_token_limit: Optional[int] = Field(None, description="Дневной лимит токенов")
    monthly_token_limit: Optional[int] = Field(None, description="Месячный лимит токенов")
...
class CampaignResponse(BaseModel):
    ...
    ai_settings: Optional[Dict[str, Any]] = Field(None, description="AI настройки кампании")
