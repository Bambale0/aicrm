...
class EmailAttachment(BaseModel):
    ...
    content_type: str = Field(default="application/octet-stream", description="MIME тип файла")
...
class TemplateEmailRequest(BaseModel):
    ...
    template_name: str = Field(..., description="Имя шаблона", examples=["order_confirmation", "task_assigned"])
    to: str | List[EmailStr] = Field(..., description="Получатель(и) email")
...
class EmailStatusResponse(BaseModel):
    ...
    email_id: Optional[str] = Field(None, description="Уникальный ID отправленного email")
...
class EmailServiceStatus(BaseModel):
    ...
    status: str = Field(..., description="Статус конфигурации", examples=["configured", "not_configured"])
