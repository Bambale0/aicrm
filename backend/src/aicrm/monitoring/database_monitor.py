        ...
        try:
            with self.engine.connect() as conn:
                ...
                stats = {
                    ...
                    'connection_utilization': round((active or 0) / (max_conn or 100) * 100, 2)
                }

                # Обновляем глобальные метрики
                metrics_service.update_db_connections_active(stats['active_connections'])
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        try:
            with self.engine.connect() as conn:
                ...
                metrics['database_size'] = db_size_result[0] if db_size_result else 'Unknown'
                metrics['database_size_bytes'] = db_size_result[1] if db_size_result else 0
            ...
            with self.engine.connect() as conn:
                ...
                if replica_count > 0:
                    ...
                    return {
                        ...
                        'replicas': replicas,
                        'max_lag_seconds': max([r['replay_lag_seconds'] for r in replicas]) if replicas else 0
                    }
                else:
                    return {
