...
class TestAutomationErrorHandlerWithNotifications:
    ...
    def mock_notification_service(self):
        ...
        with patch('src.aicrm.services.automation.error_handler.NotificationService') as mock:
            mock_instance = MagicMock()
            mock_instance.send_notification = AsyncMock(return_value={"success": True})
            mock_instance.send_system_alert = AsyncMock(return_value={"successful": 2, "failed": 0})
            mock_instance.notify_admins = AsyncMock(return_value={"successful": 2, "failed": 0})
    ...
    @pytest.mark.asyncio
    async def test_handle_error_critical_notification(self, error_handler, sample_execution, sample_robot, mock_notification_service):
        """Тест обработки критической ошибки с уведомлением"""
        # Мокаем получение робота
        error_handler.db.query.return_value.filter.return_value.first.return_value = sample_robot
    ...
    @pytest.mark.asyncio
    async def test_handle_error_with_retry_logic(self, error_handler, sample_execution, sample_robot, mock_notification_service):
    ...
    @pytest.mark.asyncio
    async def test_handle_error_non_retryable(self, error_handler, sample_execution, sample_robot, mock_notification_service):
    ...
    @pytest.mark.asyncio
    async def test_handle_error_with_custom_notification_channels(self, error_handler, sample_execution, sample_robot, mock_notification_service):
    ...
    @pytest.mark.asyncio
    async def test_handle_error_with_fallback_actions(self, error_handler, sample_execution, sample_robot, mock_notification_service):
        ...
        result = await error_handler.handle_error(
            ...
            {
                "fallback_actions": [
                    {"type": "send_manual_notification", "recipient": "manager@example.com"}
                ]
            }
        )
    ...
    async def test_can_retry_timeout_error(self, error_handler):
        ...
        assert await error_handler._can_retry("Connection timeout", {"max_retries": 3}) is True
        assert await error_handler._can_retry("Network timeout", {"max_retries": 3}) is True
    ...
    async def test_can_retry_authentication_error(self, error_handler):
        ...
        assert await error_handler._can_retry("Authentication failed", {"max_retries": 3}) is False
        assert await error_handler._can_retry("Invalid credentials", {"max_retries": 3}) is False
    ...
    async def test_can_retry_validation_error(self, error_handler):
        ...
        assert await error_handler._can_retry("Validation error", {"max_retries": 3}) is False
        assert await error_handler._can_retry("Invalid data format", {"max_retries": 3}) is False
    ...
    async def test_schedule_retry_success(self, error_handler, sample_execution):
        ...
        with patch('src.aicrm.services.automation.error_handler.asyncio.sleep', new_callable=AsyncMock) as mock_sleep:
            with patch('src.aicrm.services.automation.error_handler.AutomationService') as mock_automation_service:
                mock_service_instance = MagicMock()
                mock_service_instance.move_to_stage = AsyncMock(return_value={"success": True})
    ...
    @pytest.mark.asyncio
    async def test_execute_fallback_actions_email_fallback(self, error_handler, mock_notification_service):
    ...
    async def test_execute_fallback_actions_create_task(self, error_handler):
        ...
        with patch('src.aicrm.services.automation.error_handler.TaskService') as mock_task_service:
            mock_service_instance = MagicMock()
            mock_service_instance.create_task = AsyncMock(return_value=MagicMock(id=300))
    ...
    async def test_execute_fallback_actions_log_to_external_system(self, error_handler):
        ...
        with patch('src.aicrm.services.automation.error_handler.external_api_service') as mock_external_api:
    ...
    @pytest.mark.asyncio
    async def test_create_error_record_success(self, error_handler, sample_execution, sample_robot):
    ...
    @pytest.mark.asyncio
    async def test_create_error_record_with_actions_executed(self, error_handler, sample_execution, sample_robot):
    ...
    @pytest.mark.asyncio
    async def test_send_error_notification_success(self, error_handler, mock_notification_service):
    ...
    @pytest.mark.asyncio
    async def test_send_error_notification_failure(self, error_handler, mock_notification_service):
    ...
    async def test_bulk_error_handling(self, error_handler, mock_notification_service):
        ...
        error_handler.db.query.return_value.filter.return_value.first.return_value = robot
    ...
    def test_error_recovery_suggestions(self, error_handler):
        ...
        suggestions = {
            "timeout": ["Check network connectivity", "Increase timeout values", "Retry operation"],
            "auth": ["Verify credentials", "Check API keys", "Renew authentication token"],
            "permission": ["Check user permissions", "Verify access rights", "Contact administrator"],
            "validation": ["Check input data format", "Validate required fields", "Review data constraints"],
            "database": ["Check database connectivity", "Verify table existence", "Check database locks"],
            "network": ["Check network configuration", "Verify DNS resolution", "Test connectivity"]
        }
    ...
    def test_error_metrics_collection(self, error_handler):
        ...
        with patch('src.aicrm.services.automation.error_handler.metrics_service') as mock_metrics:
    ...
    def test_error_escalation_logic(self, error_handler):
        ...
        escalation_rules = {
            "critical": ["immediate_notification", "create_incident", "notify_management"],
