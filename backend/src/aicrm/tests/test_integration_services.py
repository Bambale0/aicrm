...
class TestServiceIntegration:
    ...
    @pytest.mark.asyncio
    async def test_error_handler_with_notification_integration(self, error_handler, notification_service, sample_execution, sample_customer):
        """Интеграционный тест: ErrorHandler + NotificationService"""
        # Мокаем получение робота
        mock_robot = MagicMock()
        mock_robot.name = "Email Robot"
        error_handler.db.query.return_value.filter.return_value.first.return_value = mock_robot

        # Мокаем создание записи об ошибке
        error_handler.db.add.return_value = None

        # Мокаем NotificationService
        with patch('src.aicrm.services.automation.error_handler.NotificationService') as mock_ns_class:
            mock_ns_instance = MagicMock()
            mock_ns_instance.send_system_alert = AsyncMock(return_value={"successful": 2, "failed": 0})
    ...
    @pytest.mark.asyncio
    async def test_avito_integration_with_error_handler(self, avito_integration, error_handler, sample_customer):
        ...
        avito_integration._analyze_message_for_tasks = AsyncMock(return_value=[
            {
                "type": "urgent_help",
        ...
        avito_integration._create_task_from_trigger = AsyncMock(side_effect=Exception("Database connection failed"))

        # Мокаем ErrorHandler
        with patch('src.aicrm.services.automation.avito_integration.AutomationErrorHandler') as mock_eh_class:
            mock_eh_instance = MagicMock()
            mock_eh_instance.handle_error = AsyncMock(return_value={
                "error_recorded": True,
            ...
            result = await avito_integration.analyze_and_create_tasks(message_data, sample_customer)

            # Проверяем что ошибка была обработана ErrorHandler
            mock_eh_instance.handle_error.assert_called_once()

    @pytest.mark.asyncio
    async def test_notification_service_with_ai_settings(self, notification_service, ai_settings_service):
        ...
        with patch.object(ai_settings_service, 'get_user_ai_settings', return_value=mock_ai_settings):
            # Мокаем email сервис
            with patch('src.aicrm.services.notification_service.email_service') as mock_email:
    ...
    @pytest.mark.asyncio
    async def test_full_error_workflow_integration(self, error_handler, notification_service, avito_integration, sample_execution, sample_customer):
        ...
        avito_integration._create_task_from_trigger = AsyncMock(side_effect=Exception("Task creation failed"))
        ...
        with patch('src.aicrm.services.automation.error_handler.NotificationService') as mock_ns_class:
            mock_ns_instance = MagicMock()
            mock_ns_instance.send_system_alert = AsyncMock(return_value={"successful": 2, "failed": 0})
            mock_ns_instance.notify_admins = AsyncMock(return_value={"successful": 2, "failed": 0})
            mock_ns_class.return_value = mock_ns_instance
    ...
    @pytest.mark.asyncio
    async def test_bulk_operations_integration(self, notification_service, ai_settings_service):
        ...
        with patch.object(ai_settings_service, 'bulk_update_settings', new_callable=AsyncMock) as mock_bulk_update:
            ...
            with patch.object(notification_service, 'send_bulk_notifications', new_callable=AsyncMock) as mock_bulk_notify:
                ...
                settings_result = await ai_settings_service.bulk_update_settings(users_settings)
    ...
    @pytest.mark.asyncio
    async def test_cascading_failure_recovery(self, error_handler, notification_service):
        ...
        # Мокаем ErrorHandler
        error_handler.db.query.return_value.filter.return_value.first.return_value = MagicMock(name="Test Robot")
        error_handler.db.add.return_value = None
    ...
    @pytest.mark.asyncio
    async def test_service_health_monitoring_integration(self, notification_service, error_handler):
        ...
        with patch.object(notification_service, 'send_system_alert', new_callable=AsyncMock) as mock_alert:
    ...
    @pytest.mark.asyncio
    async def test_user_journey_with_service_integration(self, avito_integration, notification_service, ai_settings_service, sample_customer):
        ...
        with patch.object(notification_service, 'send_notification', new_callable=AsyncMock) as mock_notify:
            mock_notify.return_value = {"success": True}

            # Шаг 4: AISettingsService проверяет настройки для персонализации
            with patch.object(ai_settings_service, 'get_user_ai_settings', return_value={
                "provider": "openai",
                "model": "gpt-4",
                "is_active": True
            }):
                # Выполняем полный путь
                tasks_created = await avito_integration.analyze_and_create_tasks(message_data, sample_customer)
    ...
    @pytest.mark.asyncio
    async def test_performance_monitoring_integration(self, error_handler, notification_service):
        ...
        with patch.object(notification_service, 'send_system_alert', new_callable=AsyncMock) as mock_alert:
            ...
            expected_alerts = ["high_response_time", "high_error_rate", "high_cpu_usage", "high_memory_usage"]
            assert mock_alert.call_count == len(expected_alerts)

    @pytest.mark.asyncio
    async def test_data_consistency_across_services(self, ai_settings_service, notification_service):
        ...
        with patch.object(ai_settings_service, 'create_ai_settings', new_callable=AsyncMock) as mock_create:
            mock_create.return_value = {**ai_settings_data, "id": 1, "is_active": True}

            # Создаем настройки
            created_settings = await ai_settings_service.create_ai_settings(ai_settings_data)
