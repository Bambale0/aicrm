...
class TestNotificationService:
    ...
    def mock_sms_service(self):
        ...
        with patch('src.aicrm.services.notification_service.sms_service') as mock:
            mock.send_sms = AsyncMock(return_value={
                "success": True,
                "provider": "smsc",
                "message_id": "msg123",
                "cost": 1.5
    ...
    def mock_telegram_service(self):
        ...
        with patch('src.aicrm.services.notification_service.TelegramBotService') as mock_class:
    ...
    @pytest.mark.asyncio
    async def test_send_email_notification_success(self, notification_service, mock_email_service):
    ...
    @pytest.mark.asyncio
    async def test_send_sms_notification_success(self, notification_service, mock_sms_service):
    ...
    @pytest.mark.asyncio
    async def test_send_telegram_notification_success(self, notification_service, mock_telegram_service):
        ...
        mock_telegram_service.send_message_to_chat.assert_called_once_with("123456789", "Test Telegram message")

    @pytest.mark.asyncio
    async def test_send_multi_channel_notification(self, notification_service, mock_email_service, mock_sms_service):
    ...
    @pytest.mark.asyncio
    async def test_send_notification_with_template(self, notification_service, mock_email_service):
    ...
    @pytest.mark.asyncio
    async def test_send_notification_partial_failure(self, notification_service, mock_email_service, mock_sms_service):
    ...
    @pytest.mark.asyncio
    async def test_send_bulk_notifications(self, notification_service, mock_email_service):
    ...
    @pytest.mark.asyncio
    async def test_notify_admins_success(self, notification_service, mock_email_service, mock_sms_service):
        ...
        assert result["total_recipients"] == 2  # admin@example.com и support@example.com
        assert result["successful"] == 2
        assert result["failed"] == 0

    @pytest.mark.asyncio
    async def test_notify_managers_success(self, notification_service, mock_email_service):
    ...
    async def test_send_task_assigned_notification(self):
        ...
        with patch('src.aicrm.services.notification_service.NotificationService') as mock_service_class:
    ...
    async def test_send_system_alert(self):
        ...
        with patch('src.aicrm.services.notification_service.NotificationService') as mock_service_class:
            mock_service = MagicMock()
            mock_service.notify_admins = AsyncMock(return_value={"successful": 2, "failed": 0})
            mock_service_class.return_value = mock_service
    ...
    def test_generate_subject_by_type_and_priority(self, notification_service):
        ...
        subject = notification_service._generate_subject(NotificationType.INFO, NotificationPriority.MEDIUM)
        assert subject == "Информация"

        # Тест срочного уведомления
        subject = notification_service._generate_subject(NotificationType.ERROR, NotificationPriority.URGENT)
        assert subject == "СРОЧНО: Ошибка"

        # Тест высокого приоритета
        subject = notification_service._generate_subject(NotificationType.WARNING, NotificationPriority.HIGH)
    ...
    def test_get_template_for_type(self, notification_service):
        ...
        template = notification_service._get_template_for_type(NotificationType.TASK_ASSIGNED)
        assert template == "task_assigned"

        template = notification_service._get_template_for_type(NotificationType.ORDER_UPDATE)
