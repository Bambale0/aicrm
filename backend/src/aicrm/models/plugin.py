...
class PluginAction(Base):
    ...
    execution_status = Column(String(50), nullable=False)  # 'success', 'error', 'running'
...
class PluginHook(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    plugin_id = Column(Integer, nullable=False, index=True)
    hook_type = Column(String(100), nullable=False, index=True)  # e.g., 'before_save', 'after_create'
    hook_event = Column(String(100), nullable=False, index=True)  # e.g., 'order_created', 'customer_updated'
    priority = Column(Integer, default=10, nullable=False)  # Execution priority (lower = higher priority)
...
class PluginPermission(Base):
    ...
    resource_type = Column(String(100), nullable=False)  # e.g., 'order', 'customer', 'email'
    resource_id = Column(String(100), nullable=True)  # Specific resource ID or '*' for all
...
class PluginRegistry(Base):
    ...
    # Classification
    category = Column(String(100), nullable=False, index=True)  # e.g., 'integration', 'automation', 'report'
    ...
    # Status
    is_official = Column(Boolean, default=False, nullable=False)  # Official AI CRM plugin
    is_featured = Column(Boolean, default=False, nullable=False)  # Featured in marketplace
