...
class SystemSettingsService:
    ...
    def update_settings(
        ...
        allowed_fields = {
            'site_name', 'site_description', 'admin_email', 'support_email',
            'session_timeout_minutes', 'max_login_attempts', 'lockout_duration_minutes',
            'password_min_length', 'require_2fa', 'enable_monitoring', 'monitoring_interval_seconds',
            'alert_email_enabled', 'performance_thresholds', 'log_level', 'log_retention_days',
            'enable_audit_log', 'redis_enabled', 'redis_host', 'redis_port', 'cache_ttl_seconds',
            'smtp_enabled', 'smtp_host', 'smtp_port', 'smtp_user', 'smtp_password',
            'smtp_use_tls', 'smtp_use_ssl', 'smtp_from_email', 'smtp_from_name',
            'telegram_enabled', 'telegram_bot_token', 'telegram_chat_id', 'telegram_webhook_url',
            'telegram_notification_enabled', 'notifications_enabled', 'email_notifications_enabled',
            'websocket_notifications_enabled', 'push_notifications_enabled', 'auto_backup_enabled',
            'backup_frequency_hours', 'backup_retention_days', 'backup_path', 'auto_update_enabled',
            'update_check_frequency_hours', 'update_channel'
        }
        ...
        result = db.execute(select(SystemSettings).where(SystemSettings.id == settings_id))
    ...
    def get_monitoring_config(db: Session) -> Dict[str, Any]:
        ...
        return {
            ...
            'performance_thresholds': settings.performance_thresholds or {'cpu_percent': 80, 'memory_percent': 85}
        }
