...
class EmailTemplateService:
    ...
    async def create_template(
        ...
        try:
            ...
            if existing:
                raise ValueError(f"Шаблон с именем '{template_data['name']}' уже существует")
    ...
    async def update_template(
        ...
        try:
            ...
            if "name" in update_data and update_data["name"] != template.name:
                ...
                if existing:
                    raise ValueError(f"Шаблон с именем '{update_data['name']}' уже существует")
    ...
    async def get_template_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            active = self.db.query(EmailTemplate).filter(EmailTemplate.is_active == True).count()
            by_category = {}

            # Статистика по категориям
            categories = self.db.query(EmailTemplate.category).distinct().all()
            for (category,) in categories:
                count = self.db.query(EmailTemplate).filter(
                    EmailTemplate.category == category
                ).count()
                by_category[category] = count

            # Самые используемые
            top_used = self.db.query(EmailTemplate).filter(
                EmailTemplate.usage_count > 0
            ).order_by(EmailTemplate.usage_count.desc()).limit(5).all()
    ...
    async def initialize_default_templates(self, created_by: int = 1) -> List[EmailTemplate]:
        """
        ...
        try:
            ...
            logger.info(f"Инициализировано {len(created_templates)} стандартных шаблонов")
    ...
    async def export_templates(self, category: Optional[str] = None) -> List[Dict[str, Any]]:
        """
            ...
            for template_data in templates_data:
                try:
                    ...
                    if existing:
                        ...
                        await self.update_template(existing.id, import_data, imported_by)
                    else:
                        # Создаем новый
