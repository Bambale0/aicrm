...
class TelegramOrderService:
    ...
    async def process_order_request(
        ...
        try:
            ...
            customer = self.db.query(Customer).filter(Customer.id == telegram_chat.customer_id).first()
            if not customer:
                return {
                    "success": False,
    ...
    async def _create_order_from_data(self, order_data: Dict[str, Any], customer: Customer) -> Order:
        """
    ...
    def _parse_deadline(self, deadline_str: str) -> Optional[datetime]:
        ...
        try:
            ...
            if "завтра" in deadline_str.lower():
                ...
                return datetime.utcnow().replace(day=datetime.utcnow().day + 2, hour=23, minute=59, second=59)
    ...
    async def _extract_order_updates(self, message: str, order: Order) -> Dict[str, Any]:
        """
        Извлечение обновлений заказа из сообщения
        """
        current_items = json.loads(order.items) if isinstance(order.items, str) else order.items
        ...
        messages = [
            {"role": "system", "content": "Анализируй запросы на обновление заказов. Отвечай в JSON."},
