...
class RateLimitService:
    ...
    async def check_rate_limit(
        ...
        try:
            ...
            if request_count >= max_requests:
                # Лимит превышен
                logger.warning("Rate limit exceeded", key=key, limit_type=limit_type, count=request_count, limit=max_requests)
                metrics_service.record_error("rate_limit_exceeded", f"{limit_type}_endpoint")
            ...
            logger.error("Rate limit check error", key=key, limit_type=limit_type, error=str(e))
    ...
    async def get_rate_limit_info(self, key: str, limit_type: str = "api") -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("Rate limit info error", key=key, limit_type=limit_type, error=str(e))
            return {
                "limit": max_requests,
    ...
    async def reset_rate_limit(self, key: str, limit_type: str = "api") -> bool:
        ...
        try:
            ...
            logger.error("Rate limit reset error", key=key, limit_type=limit_type, error=str(e))
            return False
    ...
    async def check_ai_rate_limit(self, request: Request, user_id: Optional[int] = None) -> Tuple[bool, Dict[str, Any]]:
    ...
    async def check_auth_rate_limit(self, request: Request) -> Tuple[bool, Dict[str, Any]]:
    ...
    async def check_api_rate_limit(self, request: Request, user_id: Optional[int] = None) -> Tuple[bool, Dict[str, Any]]:
        """Проверка rate limit для общих API запросов"""
        key = self.get_client_identifier(request, user_id)
        return await self.check_rate_limit(key, "api")

    async def check_global_rate_limit(self, request: Request) -> Tuple[bool, Dict[str, Any]]:
...
async def rate_limit_dependency(
    ...
    async with service:
        allowed, limit_info = await service.check_rate_limit(
            service._get_client_ip(request),
            limit_type
        )
