...
# AI Usage Metrics
AI_REQUESTS = Counter('ai_requests_total', 'Total AI API requests', ['model', 'provider', 'status'])
AI_TOKENS_USED = Counter('ai_tokens_total', 'Total AI tokens used', ['model', 'direction'])
AI_RESPONSE_TIME = Histogram('ai_response_time_seconds', 'AI response time', ['model'])

# Communication Metrics
COMMUNICATIONS_SENT = Counter('communications_sent_total', 'Communications sent', ['channel', 'direction'])
COMMUNICATIONS_RECEIVED = Counter('communications_received_total', 'Communications received', ['channel'])
EMAIL_SEND_SUCCESS = Counter('email_send_success_total', 'Successful email sends')
EMAIL_SEND_FAILURE = Counter('email_send_failure_total', 'Failed email sends', ['error_type'])
...
PRODUCTION_STEPS_COMPLETED = Counter('production_steps_completed_total', 'Production steps completed')
...
DB_OPERATIONS_TOTAL = Counter('db_operations_total', 'Database operations', ['operation', 'table'])

# Automation Metrics
AUTOMATION_EXECUTIONS = Counter('automation_executions_total', 'Automation executions', ['process_type', 'status'])
AUTOMATION_ERRORS = Counter('automation_errors_total', 'Automation execution errors', ['error_type'])
...
VALIDATION_ERRORS = Counter('validation_errors_total', 'Validation errors', ['field', 'endpoint'])
...
EXTERNAL_API_REQUESTS = Counter('external_api_requests_total', 'External API requests', ['service', 'status'])
...
class MetricsService:
    ...
    def _init_business_metrics(self):
        ...
        # User metrics
        self.user_activity_summary = Summary('user_session_duration_seconds', 'User session duration')

        # Business KPIs
        self.business_revenue = Counter('business_revenue_rub', 'Business revenue in RUB')
        self.customer_satisfaction = Gauge('customer_satisfaction_score', 'Customer satisfaction score (1-5)')

        # Workflow efficiency
        self.workflow_completion_rate = Gauge('workflow_completion_rate_percent', 'Workflow completion rate')
    ...
    @staticmethod
    def record_ai_request(model: str, provider: str, status: str, tokens: Optional[int] = None):
    ...
    def get_business_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'communications_total': sum(COMMUNICATIONS_SENT._value) + sum(COMMUNICATIONS_RECEIVED._value),
