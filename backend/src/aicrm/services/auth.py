...
class AuthService:
    ...
    @staticmethod
    async def authenticate_user_async(db: Session, email: str, password: str) -> Optional[User]:
        """Асинхронная версия аутентификации пользователя"""
        return await asyncio.to_thread(AuthService.authenticate_user, db, email, password)
    ...
    @staticmethod
    async def login_with_session(db: Session, email: str, password: str, request: Request = None) -> Optional[Dict[str, Any]]:
    ...
    @staticmethod
    async def get_current_user_from_session(session_id: str, db: Session) -> Optional[User]:
        ...
        if user:
            logger.debug("User retrieved from session", user_id=user_id, session_id=session_id)
    ...
    async def logout_all_sessions(user_id: int) -> int:
        ...
        logger.info("User logged out from all sessions", user_id=user_id, sessions_count=count)
        return count
