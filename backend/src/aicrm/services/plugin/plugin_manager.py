...
from .plugin_interfaces import (
    BasePlugin, ActionPlugin, HookPlugin, HookContext, HookResult,
...
class PluginManager:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            plugins_query = self.db.query(Plugin).filter(
            ...
            logger.info(f"Plugin system initialized: {loaded_count} loaded, {failed_count} failed")
            return True
    ...
    async def shutdown(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Plugin system shut down: {shutdown_count} shut down, {failed_count} failed")
            return True
    ...
    async def load_plugin(self, plugin_record: Plugin) -> bool:
        ...
        try:
            ...
            if not validation['is_valid']:
                logger.error(f"Plugin '{plugin_name}' validation failed: {validation['errors']}")
                return False
    ...
    async def unload_plugin(self, plugin_name: str) -> bool:
        ...
        try:
            ...
            if not shutdown_success:
                logger.warning(f"Plugin '{plugin_name}' shutdown may not have completed properly")
            ...
            plugin_record = self.db.query(Plugin).filter(Plugin.name == plugin_name).first()
            if plugin_record:
    ...
    async def install_plugin(
        ...
        try:
            ...
            plugin_info = plugin_class().info if hasattr(plugin_class(), 'info') else None
            if not plugin_info:
                return {
                    'success': False,
                    'error': 'Plugin must provide info property'
                }
    ...
    async def uninstall_plugin(self, plugin_name: str, user_id: Optional[int] = None) -> Dict[str, Any]:
        """
        Uninstall a plugin
        ...
        try:
            ...
            self.db.query(PluginAction).filter(PluginAction.plugin_id == plugin_record.id).delete()
            self.db.query(PluginHook).filter(PluginHook.plugin_id == plugin_record.id).delete()
            self.db.query(PluginPermission).filter(PluginPermission.plugin_id == plugin_record.id).delete()
    ...
    def get_plugin_registry(self) -> List[Dict[str, Any]]:
        ...
        """
        registry_plugins = self.db.query(PluginRegistry).filter(
    ...
    async def _register_plugin_hooks(self, plugin_id: int, plugin_instance: HookPlugin):
        ...
        for hook in hooks:
            ...
            try:
                ...
                logger.error(f"Failed to register hook for plugin {plugin_instance.info.name}: {e}")

        self.db.commit()

    async def _register_plugin_actions(self, plugin_id: int, plugin_instance: ActionPlugin):
    ...
    def _check_hook_conditions(self, conditions: Optional[Dict[str, Any]], context: HookContext) -> bool:
