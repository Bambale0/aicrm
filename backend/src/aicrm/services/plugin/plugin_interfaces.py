...
def validate_plugin_implementation(plugin_class: type) -> Dict[str, Any]:
    ...
    required_methods = ['info', 'initialize', 'shutdown', 'get_settings_schema', 'get_default_settings']
    ...
    try:
        ...
        result['warnings'].append("Plugin class should not be instantiable without configuration")
    except Exception:
    ...
    for interface_name, interface_class in interfaces:
        required_methods = [m for m in dir(interface_class) if not m.startswith('_') and callable(getattr(interface_class, m))]
        implements_all = True
