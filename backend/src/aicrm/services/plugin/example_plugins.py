...
from .plugin_interfaces import (
    BasePlugin, ActionPlugin, HookPlugin, AutomationPlugin,
    PluginInfo, HookContext, HookResult, create_plugin_info
)
...
class ExampleActionPlugin(ActionPlugin):
    ...
    async def execute_action(
        ...
        print(f"[ExampleActionPlugin] Executing action: {action_name} with params: {parameters}")
...
class ExampleHookPlugin(HookPlugin):
    ...
    async def handle_hook(self, hook_context: HookContext) -> HookResult:
        ...
        print(f"[ExampleHookPlugin] Handling hook: {hook_context.hook_type}:{hook_context.hook_event}")
...
class ExampleAutomationPlugin(AutomationPlugin):
    ...
    def get_default_settings(self) -> Dict[str, Any]:
        """Return default settings"""
        return {
            "custom_sms_provider": "provider_a",
            "custom_sms_api_key": ""
        }
        ...
        return [
            ...
            },
            {
                ...
                "parameters_schema": {
                    "type": "object",
                    "properties": {
                        "channel": {"type": "string", "title": "Slack channel", "default": "#general"},
