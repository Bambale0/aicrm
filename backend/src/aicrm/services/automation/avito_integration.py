...
class AvitoTaskIntegration:
    ...
    async def analyze_and_create_tasks(self, message_data: Dict[str, Any], customer: Optional[Customer] = None) -> List[Task]:
        """
        ...
        try:
            ...
            task_triggers = await self._analyze_message_for_tasks(message_text, customer, item_id)

            for trigger in task_triggers:
                task = await self._create_task_from_trigger(trigger, customer, chat_id, item_id)
                if task:
                    created_tasks.append(task)
                    logger.info(f"Создана задача {task.id} на основе сообщения Avito: {trigger['type']}")
    ...
    async def _analyze_message_for_tasks(self, message_text: str, customer: Optional[Customer], item_id: Optional[int]) -> List[Dict[str, Any]]:
        """
        ...
        for task_type, config in task_patterns.items():
            if any(keyword in message_text for keyword in config["keywords"]):
                triggers.append({
                    ...
                    "matched_keywords": [kw for kw in config["keywords"] if kw in message_text]
                })
    ...
    async def _create_task_from_trigger(
        ...
        try:
            ...
            logger.info(f"Создана задача {task.id} типа '{trigger['type']}' для пользователя {assigned_user_id}")
            return task
    ...
    async def _determine_task_assignee(self, trigger: Dict[str, Any], customer: Optional[Customer]) -> Optional[int]:
        """
    ...
    async def create_followup_task(self, customer: Customer, chat_id: str, reason: str) -> Optional[Task]:
        """
        ...
        try:
            task_data = {
                ...
                "assigned_to": await self._determine_task_assignee({"priority": "medium"}, customer),
                "priority": "medium",
            ...
            logger.info(f"Создана задача follow-up {task.id} для клиента {customer.name}")
            return task

        except Exception as e:
            logger.error(f"Ошибка создания задачи follow-up для клиента {customer.id}: {e}")
            return None

    async def create_order_tracking_task(self, order: Order, customer: Customer, chat_id: str) -> Optional[Task]:
        """
        ...
        try:
            ...
            logger.info(f"Создана задача отслеживания заказа {task.id} для заказа #{order.id}")
            return task

        except Exception as e:
            logger.error(f"Ошибка создания задачи отслеживания для заказа {order.id}: {e}")
            return None
    ...
    async def get_tasks_for_chat(self, chat_id: str) -> List[Task]:
        ...
        try:
            tasks = self.db.query(Task).filter(
                Task.extra_data.contains({"chat_id": chat_id})
            ).all()
    ...
    async def update_task_status_from_avito(self, chat_id: str, new_status: str, updated_by: int = 1) -> bool:
        """
        ...
        try:
            ...
            if updated_count > 0:
                logger.info(f"Обновлено {updated_count} задач для чата {chat_id} на статус '{new_status}'")
    ...
    async def notify_new_lead(self, customer: Customer, chat_id: str, item_id: Optional[int] = None) -> bool:
        """
        ...
        try:
            ...
            task_data = {
                ...
                "assigned_to": await self._determine_task_assignee({"priority": "high"}, customer),
                "priority": "high",
                "customer_id": customer.id,
                "extra_data": {
            ...
            await self._notify_managers_about_new_lead(customer, chat_id, item_id, lead_task)

            logger.info(f"Создано уведомление о новом лиде {customer.name}, задача #{lead_task.id}")
            return True
    ...
    async def _notify_managers_about_new_lead(
        ...
        try:
            ...
            managers = self.db.query(User).filter(User.role.in_(["manager", "admin"])).all()
            ...
            for manager in managers:
                try:
                    ...
                    logger.error(f"Не удалось уведомить менеджера {manager.id} о новом лиде: {e}")

            logger.info(f"Отправлены уведомления {len(managers)} менеджерам о новом лиде {customer.name}")

        except Exception as e:
            logger.error(f"Ошибка отправки уведомлений менеджерам о лиде {customer.id}: {e}")

    async def create_lead_followup_reminder(self, customer: Customer, chat_id: str, hours_delay: int = 24) -> Optional[Task]:
        """
        ...
        try:
            ...
            logger.info(f"Создано напоминание follow-up #{task.id} для лида {customer.name}")
            return task

        except Exception as e:
            logger.error(f"Ошибка создания напоминания follow-up для лида {customer.id}: {e}")
            return None
