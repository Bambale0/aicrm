...
class AutomationErrorHandler:
    ...
    async def handle_error(
        ...
        try:
            ...
            log_entry = await self._create_error_log_v2(
                automation_execution_id, robot_id, trigger_id,
    ...
    async def _create_error_log(
        ...
        try:
            ...
            from ...services.automation.automation_log_service import AutomationLogService
    ...
    def _extract_error_details(self, error: Exception, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
    ...
    async def _send_error_notifications(
        ...
        try:
            ...
            channels, recipients = self._get_notification_targets(severity, automation_type)
...
Требуется внимание разработчиков для исправления проблемы."""
    ...
    def _get_notification_targets(
        ...
        if severity == ErrorSeverity.CRITICAL:
            critical_contacts = [
                {"email": "ceo@example.com", "phone": "+1234567890"}
            ]
    ...
    def _map_severity_to_priority(self, severity: ErrorSeverity) -> NotificationPriority:
        """
    ...
    async def get_error_history(
        ...
        try:
            ...
            for log in logs:
                ...
                if automation_type and details.get("automation_type") != automation_type:
                    continue
    ...
    async def get_error_stats(
        ...
        try:
            ...
            for log in logs:
                ...
                stats["by_automation_type"][automation_type] = stats["by_automation_type"].get(automation_type, 0) + 1
    ...
    async def resend_error_notification(self, log_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if not log_entry or log_entry.level != "error":
                return {"success": False, "error": "Запись не найдена или не является ошибкой"}
    ...
    async def _check_retry_needed(
        ...
        retryable_errors = [
            "network_error", "timeout_error", "api_rate_limit",
            "temporary_service_unavailable", "connection_error"
        ]

        # Ошибки, которые нельзя повторять
        non_retryable_errors = [
            "authentication_error", "authorization_error",
            "validation_error", "not_found_error", "permission_denied"
        ]
