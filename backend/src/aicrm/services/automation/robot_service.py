...
from ...models.automation import Robot, RobotAction, RobotActionConfig, EntityType
...
class RobotService:
    ...
    async def _execute_robot_sequence(
        ...
        try:
            ...
            logger.error(f"Failed to create automation execution for robot {robot.id}: {e}")
            raise

        logger.info(f"Automation execution created: ID {automation_execution.id} for robot {robot.id}")
        ...
        for action_config in robot.actions:
            try:
                ...
                is_critical = self._is_error_critical(error_message, action_config.action_type)
                if is_critical:
                    logger.warning(f"Critical error in robot {robot.id}, stopping execution")
                    break
        ...
        try:
            ...
            if total_executed > 0:
                ...
                automation_execution.execution_status = "completed" if success_rate == 1.0 else "partially_completed"
            else:
                automation_execution.execution_status = "completed"  # No actions to execute
            self.db.commit()
        except Exception as e:
    ...
    async def _execute_send_telegram(
        ...
        try:
            ...
            return {
                "status": "telegram_sent" if result.get("success") else "telegram_failed",
    ...
    async def _execute_create_message(
        ...
        communication = Communication(
            ...
            customer_id=entity_data.get("customer_id") if entity_type == EntityType.CUSTOMER else None,
    ...
    async def _execute_notify_group(
        ...
        for user in group_users:
            try:
                result = await self._notify_single_user(user, message, notification_type, channel)
        ...
        logger.info(f"Group notification completed: {notified_count} notified, {failed_count} failed")
    ...
    async def _execute_create_communication(
        ...
        communication = Communication(
            ...
            message_content=self._replace_placeholders(
                config.get("content", "Автоматическая коммуникация"),
                entity_data
            ),
            message_type=config.get("message_type", "notification"),
            customer_id=entity_data.get("customer_id") if entity_type == EntityType.CUSTOMER else None,
        ...
        logger.info(f"Communication record created: ID {communication.id}, channel {communication.channel}")
        ...
        return {
            ...
            "content": communication.message_content[:100] + "..." if len(communication.message_content or "") > 100 else communication.message_content,
    ...
    async def _execute_generate_response(
        ...
        try:
            ...
            response = await ai_service.generate_response(intent, text_to_analyze, {"context": {"context": context_str}})
...
Команда Company
            ...
            for key, value in data.items():
                ...
                if isinstance(template.get("subject"), str):
                    template["subject"] = template["subject"].replace(placeholder, str(value) if value is not None else "")
                if isinstance(template.get("body"), str):
                    template["body"] = template["body"].replace(placeholder, str(value) if value is not None else "")
                if isinstance(template.get("html_body"), str):
                    template["html_body"] = template["html_body"].replace(placeholder, str(value) if value is not None else "")
    ...
    async def _get_recipient_data(self, entity_type: EntityType, entity_id: int, channel: str) -> Dict[str, Any]:
        ...
        try:
            if channel == "email":
                ...
                if entity_type == EntityType.CUSTOMER:
                    customer = self.db.query(Customer).filter(Customer.id == entity_id).first()
                    telegram_id = getattr(customer, 'telegram_id', None) if customer else None
                    return {"telegram_id": telegram_id} if telegram_id else {}
                return {}
            else:
                return {}
        except Exception as e:
            logger.error(f"Failed to get recipient data for {entity_type} {entity_id} via {channel}: {e}")
            return {}

    async def _send_via_channel(self, channel: str, response: str, recipient_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _get_entity_text(self, entity_type: EntityType, entity_id: int) -> str:
        ...
        try:
            ...
            logger.error(f"Failed to get entity text for {entity_type} {entity_id}: {e}")
        ...
        try:
            ...
            if entity_type == EntityType.CUSTOMER:
                ...
                if customer:
                    ...
                    orders = self.db.query(Order).filter(Order.customer_id == entity_id).all()
                    ...
                    recent_communications = self.db.query(Communication).filter(
                        Communication.customer_id == entity_id
                    ).order_by(Communication.created_at.desc()).limit(3).all()

                    if recent_communications:
                        context_parts.append("Последние взаимодействия:")
                        for comm in recent_communications:
                            context_parts.append(f"  - {comm.channel}: {comm.message_content[:50]}..." if len(comm.message_content or "") > 50 else f"  - {comm.channel}: {comm.message_content}")
                ...
                if order:
                    ...
                    context_parts.append(f"Статус: {order.status.value if order.status else 'Неизвестен'}")

                    if order.customer:
                        context_parts.append(f"Клиент: {order.customer.name or 'Неизвестный'}")
                        context_parts.append(f"Email клиента: {order.customer.email or 'Не указан'}")
                    ...
                    if order.created_at:
                        context_parts.append(f"Дата создания: {order.created_at.strftime('%Y-%m-%d %H:%M')}")

                    # Получаем этапы производства
                    steps = self.db.query(ProductionStep).filter(ProductionStep.order_id == order.id).all()
                    if steps:
                        completed_steps = sum(1 for step in steps if step.status == "completed")
                        in_progress_steps = sum(1 for step in steps if step.status == "in_progress")
                        context_parts.append(f"Этапы производства: {completed_steps}/{len(steps)} завершено, {in_progress_steps} в работе")
                        ...
                        for step in sorted(steps, key=lambda s: s.sequence_number):
                            ...
                            context_parts.append(f"  {status_emoji} {step.name} ({step.estimated_hours}ч)")

                    # Получаем связанные задачи
                    tasks = self.db.query(Task).filter(Task.order_id == order.id).all()
                    if tasks:
                        completed_tasks = sum(1 for task in tasks if task.status == "completed")
                        context_parts.append(f"Связанные задачи: {completed_tasks}/{len(tasks)} завершено")
                ...
                if task:
                    ...
                    context_parts.append(f"Статус: {task.status.value if task.status else 'Неизвестен'}")
                    context_parts.append(f"Приоритет: {task.priority or 'Не указан'}")

                    if task.assigned_to and task.assigned_user:
                        context_parts.append(f"Исполнитель: {task.assigned_user.name or 'Неизвестен'}")

                    if task.description:
                        context_parts.append(f"Описание: {task.description}")

                    if task.deadline:
                        context_parts.append(f"Дедлайн: {task.deadline.strftime('%Y-%m-%d %H:%M')}")

                    if task.created_at:
                        context_parts.append(f"Дата создания: {task.created_at.strftime('%Y-%m-%d %H:%M')}")

                    if task.order:
                        context_parts.append(f"Связан с заказом: #{task.order.id} ({task.order.status.value if task.order.status else 'Неизвестен'})")
            ...
            context_parts.append(f"Время запроса: {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S UTC')}")

            return " | ".join(context_parts)

        except Exception as e:
            logger.error(f"Failed to get entity context for {entity_type} {entity_id}: {e}")
        ...
        try:
            ...
            if group_identifier in role_groups:
                ...
                try:
                    ...
                    logger.warning(f"Numeric group IDs not implemented yet, got: {numeric_id}")
                    return []
                except ValueError:
            ...
            logger.error(f"Failed to get group users for group '{group_identifier}': {e}")
            return []
    ...
    async def create_communication_message(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Создание сообщения в системе коммуникаций"""
        try:
            ...
            logger.info(f"Communication message created: ID {communication.id}, channel {channel}")
    ...
    async def create_production_step(self, order_id: int, step_name: str, **kwargs) -> Dict[str, Any]:
    ...
    async def notify_group(self, group_id: int, message: str, **kwargs) -> Dict[str, Any]:
    ...
    async def generate_response(self, context: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if entity_type and entity_id:
                from ...models.automation import EntityType as AutomationEntityType
                entity_type_enum = AutomationEntityType(entity_type)
                additional_context = await self._get_entity_context(entity_type_enum, entity_id)
            ...
            try:
                ...
                response = await ai_service.generate_response(intent, text, {"context": context})
                ...
                logger.warning(f"AI response generation failed, using template: {ai_error}")
    ...
    async def send_via_channel(self, channel: str, response: str, recipient_data: Dict[str, Any]) -> Dict[str, Any]:
        """Отправка через соответствующий канал"""
        return await self._send_via_channel(channel, response, recipient_data)

    async def render_email_template(self, template_name: str, context: Dict[str, Any]) -> Dict[str, Any]:
        """Рендеринг шаблона email"""
        try:
    ...
    async def get_entity_context(self, entity_type: str, entity_id: int) -> Dict[str, Any]:
