...
try:
    ...
    print("Install with: pip install psycopg2-binary redis sqlalchemy psutil memory-profiler")
    sys.exit(1)
...
class PerformanceReporter:
    ...
    def generate_report(self, results: Dict[str, Any]) -> str:
        ...
        if "cache" in results:
            ...
            report.append(f"- **Connected Clients**: {cache_results.get('connected_clients', 0)}")
            report.append(f"- **Used Memory**: {cache_results.get('used_memory', 'unknown')}")
        ...
        if "memory" in results:
            ...
            if memory_results.get("success"):
                ...
                report.append(f"❌ Memory profiling failed: {memory_results.get('error', 'Unknown error')}")
            report.append("")
...
def main():
    ...
    parser.add_argument(
        "--action",
        choices=["db_analyze", "slow_queries", "memory_profile", "stress_test", "load_test_setup", "cache_analysis", "full_report"],
        required=True,
        help="Action to perform"
    )
    parser.add_argument("--db-url", default="postgresql://user:password@localhost:5432/aicrm", help="Database URL")
    parser.add_argument("--redis-url", default="redis://localhost:6379/0", help="Redis URL")
    parser.add_argument("--output", default="performance_report.md", help="Output file for reports")
    ...
    try:
        ...
        if args.action in ["slow_queries", "full_report"]:
            results["database"]["slow_queries"] = db_analyzer.analyze_slow_queries(args.limit)
        ...
        if results:
            ...
            if "database" in results:
                ...
                print(f"📋 Tables without indexes: {len(db_res.get('tables_without_indexes', []))}")
                print(f"⚠️  Missing FK indexes: {len(db_res.get('missing_indexes', []))}")
