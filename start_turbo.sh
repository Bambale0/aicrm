#!/bin/bash
# Start script for AI CRM application with Turbopack

set -e  # Exit on any error

echo "🚀 Запуск AI CRM системы с Turbopack..."
...
# 3. Установка зависимостей с Turbopack
echo "📦 Установка зависимостей..."
cd /root/aicrm
npm run install:all

# 4. Запуск backend (FastAPI) с правильным PYTHONPATH
echo "🔧 Запуск backend (FastAPI)..."
cd /root/aicrm/backend/src

# Установка переменных окружения для backend
export HOST=${HOST:-0.0.0.0}
export PORT=${PORT:-8000}
export PYTHONPATH=/root/aicrm/backend/src:$PYTHONPATH
...
# 5. Запуск frontend (React) с Turbopack
echo "🎨 Запуск frontend (React) через Turbopack..."
cd /root/aicrm/frontend

# Очистка переменных окружения, которые могут конфликтовать с React
unset HOST
export PORT=3000

# Запуск React dev server через npm script в фоне
npm run dev &
FRONTEND_PID=$!
...
echo ""
echo "🎉 AI CRM система успешно запущена с Turbopack!"
echo "📊 Backend (FastAPI): http://localhost:$PORT"
echo "🎨 Frontend (React): http://localhost:3000"
echo "📋 API Docs: http://localhost:$PORT/docs"
echo "🔧 Turbopack: Использует кеширование и параллельную сборку"
echo ""
echo "💡 Для остановки используйте: ./stop_turbo.sh"
echo ""
echo "📝 PID процессов сохранены в /tmp/aicrm_*.pid"
echo "📋 Логи Turbopack: .turbo/turbo-*.log"

# Альтернативный способ запуска через Turbopack (комментирован)
echo ""
echo "🔄 Альтернативно можно запустить через Turbopack:"
echo "npm run dev  # Запуск через Turbopack"

# Ожидание завершения (Ctrl+C для остановки)
trap 'echo ""; echo "🛑 Получен сигнал остановки..."; ./stop_turbo.sh; exit 0' INT TERM
wait
